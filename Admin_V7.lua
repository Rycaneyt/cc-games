...
-- ASCII-only. Zweiphasiger Login mit Weiter-Button, Logs mit Zurueck-Button,
-- Dienstprogramme klickbar mit je eigener Aktion. QoL: sauberes Keyboard-Clearing,
-- Bildschirm-Clear vor Validierung.
-- CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "Koenig Fabio"
local VERSION = "FabIO-AdminOS v2.5"
...
local function clearKeyboardArea(yStart)
  ...
  clearTouchAreas() -- Touch-Zonen fuer Keyboard weg; rufe danach deine Draw-Funktionen neu
end
...
-------------------- Aktionen (seriös mit kleinem Gag am Ende) --------------------
local function explosionAnimation()
  ...
  term.setTextColor(colors.white); addLog("[ACTION] Kritischer Vorgang angefragt; durch Richtlinie blockiert.")
  sleep(1.2)
  centerText(10,"Hinweis: Kaffeevorrat bleibt unversehrt.")
  sleep(1.2)
end
...
local function hackMojangAnimation()
  ...
  sleep(1.2)
  centerText(10,"Zusatz: Kein Creeper-Verkehr festgestellt.")
  sleep(1.0)
end
...
local function interaktiveTanzroutine()
  ...
  term.setTextColor(colors.white); addLog("[INFO] Interaktives Modultest-Pattern gespielt."); sleep(1.1)
  centerText(14,"Notiz: Publikum zaehlbar auf einer Hand.")
  sleep(1.0)
end

-------------------- Dienstprogramme (klickbare Aktionen) --------------------
local function pressToContinue(y)
  local label="[ OK ]"
  centerText(y, label)
  ...
  while true do
    ...
    if ev=="monitor_touch" then local id=handleTouch(a,b); if id=="dlg_ok" then return end end
  end
end
...
local function toolCookieRule()
  ...
  term.setCursorPos(3,6); print("- Lesen lokaler Keks-Bestaende ...")
  loadingDots(32,6,0.9)
...
local function toolEgoCalibration()
  ...
  term.setCursorPos(3,6); print("- Ausgangspegel erfassen ...")
  loadingDots(32,6,0.9)
...
local function toolDancefloor()
  ...
  term.setCursorPos(3,6); print("- Initialisiere Muster ...")
  loadingDots(32,6,0.8)
  -- pseudo-visual
  for row=8,12 do
    term.setCursorPos(3,row)
    for col=1,math.min(w-6,48) do
      term.write(((row+col)%2==0) and "#" or ".")
    end
    sleep(0.05)
  end
...
local function toolAutoFeedback()
  ...
  term.setCursorPos(3,6); print("- Sammle Telemetrie ...")
  loadingDots(32,6,0.9)
...
local function utilitiesMenu()
  while true do
    ...
    local ev,a,b = waitKeyOrTouch()
    local sel = nil
    if ev=="char" and a>='1' and a<='5' then sel = tonumber(a)
    elseif ev=="key" and a==keys.enter then sel = 5
    elseif ev=="monitor_touch" then
      local id=handleTouch(a,b)
    ...
    elseif ev=="char" and (a=="+" or a=="-") then changeScale(a=="+" and SCALE_STEP or -SCALE_STEP)
    elseif ev=="term_resize" then -- redraw loop
    end
...
local function mainMenu()
  while true do
    ...
    local baseY=6
    local entries={
      "Sicherheitsprotokoll 7B testen",  -- (ehem. Selbstzerstoerung)
      "Remote-Diagnose",
    ...
    elseif sel=="6" then
      -- Geplanter Neustart (leicht ernst)
      clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
      ...
      centerText(8,"Hinweis: Kaffee bleibt heiss.")
      sleep(1.0)
    ...
    elseif sel=="8" or sel=="0" then
      ...
      centerText(6,"Abmeldung ...")
      safePlayNote("C","pling",0.7); sleep(1.5); term.clear(); return
    end
  end
end
...
-- Phase 1: Intro/Status mit Weiter-Button ODER Auto-Weiter
local function loginIntro()
  ...
  addTouchArea(x,y,x+#label-1,y,"login_next",function() end)
  ...
  while true do
    ...
    if ev=="monitor_touch" then local id=handleTouch(a,b); if id=="login_next" then break end end
    if ev=="timer" and a==timerId then break end
    if ev=="term_resize" then
      resizeCache(); clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
      term.setTextColor(colors.lightBlue); centerText(3,"== AUTHENTICATION REQUIRED =="); term.setTextColor(colors.white)
      banner(4); drawLoginStatusPanel(8)
      x = math.floor((w - #label)/2)+1; y = h-2; term.setCursorPos(x,y); term.write(label)
      addTouchArea(x,y,x+#label-1,y,"login_next",function() end)
      centerText(h-1, hint)
    end
  end
...
-- Phase 2: Formular mit Keyboard, Keyboard-Clear zwischen Name/Passwort
local function loginForm()
  ...
  -- Tastatur fuer Name-Eingabe entfernen, bevor Passwort-Keyboard erscheint
  clearKeyboardArea(kbY1)
  -- Header/Buttons wieder zeichnen, da clearKeyboardArea TouchAreas geloescht hat
  drawScaleLabelAndButtons()
