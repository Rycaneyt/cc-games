-- entertainment_story.lua  (v2.1 - Fixed Buttons)
...
local function makeButtons(x1, y1, x2, y2, options)
  ...
  for i, opt in ipairs(options) do
    ...
    table.insert(buttons, {x1, y, x2, bY2, opt.label, i, opt.hint})
    y = bY2 + 1
  end
end
...
local function drawButtons(theme)
  for _, b in ipairs(buttons) do
    ...
    term.setCursorPos(cx(text), midY)
    term.write(tostring(idx))
    -- Rest vom Label
    setc(theme.fg, theme.bg)
    term.setCursorPos(cx(text) + 2, midY)
    term.write(") " .. (label or ""))
  end
end
