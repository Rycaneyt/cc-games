-- entertainment_v3.lua  (v3.1)
-- CC:Tweaked – Entertainment Station Story Adventure (All-in-One)
-- ASCII-safe; Full Touch UI; Save Slots; Map; Dialog+Portrait; Achievements; Puzzles; Ambient; Shader; Back-Button-Fix
...
local function smoothScroll(_, _, dy, delay)
  delay = delay or 0.01
  if dy==0 then return end
  for _=1,math.abs(dy) do
    term.scroll(dy>0 and 1 or -1)
    sleep(delay)
  end
end

-- Particles
local function particles(_)
  local th=theme()
...
-- Confetti (Fix: fehlte in v3.0)
local function confetti(th, duration)
  local t0=os.clock()
  local cols={th.good, th.neutral, th.accent, th.bad}
...
local function drawMap()
  ...
  drawBox(backBtn[1],backBtn[2],backBtn[3],backBtn[4],th.frame,th.bg); term.setCursorPos(backBtn[1]+2,backBtn[2]+1); setc(th.fg,th.bg); term.write("< Zurueck")
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" and b>=backBtn[1] and b<=backBtn[3] and c>=backBtn[2] and c<=backBtn[4] then return end
...
local function drawPortrait(x,y,mood) local th=theme(); local art=PORTRAIT[mood] or PORTRAIT.neutral
...
-------------------- MENUS (Theme/Scale with Back) ----
-- FIX: robuster, eigener Zurueck-Button (unten links), unabhängig von Optionsliste
local function themeMenu()
  ...
  while true do
    ...
    term.setCursorPos(backBtn.x1 + 2, backBtn.y1 + 1); setc(th.fg, th.bg); term.write("< Zurueck")
    ...
    while true do
      ...
      if e == "monitor_touch" then
        ...
        if mx >= backBtn.x1 and mx <= backBtn.x2 and my >= backBtn.y1 and my <= backBtn.y2 then
          playSound("minecraft:block.note_block.pling", CONFIG.sfx * CONFIG.master); return
        end
        for _, bt in ipairs(buttons) do
          ...
          if mx >= x1b and mx <= x2b and my >= y1b and my <= y2b then
            CONFIG.theme = label; saveConfig(); th = theme()
...
local function scaleMenu()
  ...
  while true do
    ...
    for i = 2, #scales do table.insert(opts, { label = ("Skalierung %.1f"):format(scales[i]) }) end
    ...
    while true do
      ...
      if e == "monitor_touch" then
        ...
        for idx, bt in ipairs(buttons) do
          ...
          if mx >= x1b and mx <= x2b and my >= y1b and my <= y2b then
            playSound("minecraft:block.note_block.pling", CONFIG.sfx * CONFIG.master)
            if idx == 1 then applyAutoScale()
            else
              local val = scales[idx]
              if type(val) == "number" then CONFIG.textScale = val; mon.setTextScale(val); saveConfig() end
            end
            W, H = term.getSize(); th = theme()
            break
          end
        end
        break
