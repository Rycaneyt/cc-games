-- entertainment_v3.lua  (v3.0)
-- CC:Tweaked – Entertainment Station Story Adventure (All-in-One)
-- ASCII-safe; Full Touch UI; Save Slots; Map; Dialog+Portrait; Achievements; Puzzles; Ambient; Shader
...
local function findBiggestMonitor()
  local mons = {}
  if peripheral and peripheral.find then for _,m in ipairs({peripheral.find("monitor")}) do table.insert(mons, m) end end
...
if not mon then print("Kein Monitor. Bitte Advanced-Monitor anschliessen."); return end
...
local function playSound(id, vol, pit) if not sfx_ok() then return end; pcall(function() speaker.playSound(id,{volume=vol or 0.7, pitch=pit or 1.0}) end) end
...
local THEMES = {
  ...
  Sunset={bg=colors.black, fg=colors.white, frame=colors.gray,   titleBg=colors.orange,titleFg=colors.black, statusBg=colors.brown, statusFg=colors.white,hlBg=colors.red,   hlFg=colors.white, btnNum=colors.yellow, accent=colors.magenta,   good=colors.orange,bad=colors.red, neutral=colors.yellow},
...
-- Save State (per Slot)
local state = {
  slot=1, playerName="", scene="intro", courage=0, wisdom=0, empathy=0, level=1, xp=0,
  flags={}, decisions=0, endings={}, timeStart=os.epoch("utc"), playTime=0, mapLoc="Gate",
  journal={summary={}, quests={}, loreUnlocked={}, notes={}}, inventory={},
  achievements={}, autosave=true, lastRecap="",
  portraitMood="neutral", portraitTick=0,
}

-------------------- UTIL / DRAW -----------------------
local function saveConfig() local h=fs.open(CONFIG_FILE,"w"); if h then h.write(textutils.serialize(CONFIG)); h.close() end end
local function loadConfig() if not fs.exists(CONFIG_FILE) then return end; local h=fs.open(CONFIG_FILE,"r"); local s=h.readAll(); h.close(); local ok,t=pcall(textutils.unserialize,s); if ok and type(t)=="table" then for k,v in pairs(t) do CONFIG[k]=v end end end
loadConfig()
...
local function drawBox(x1,y1,x2,y2,fg,bg) for y=y1,y2 do hline(y,x1,x2," ",nil,bg) end; setc(fg,bg)
  term.setCursorPos(x1,y1); term.write("+"); term.setCursorPos(x2,y1); term.write("+"); term.setCursorPos(x1,y2); term.write("+"); term.setCursorPos(x2,y2); term.write("+")
...
-- Shader Light Mode (simple gradient)
local function shaderPass()
  if not CONFIG.shader then return end
  local th=theme()
  for y=4,H-2 do
    local shade = (y%4==0) and th.statusBg or th.bg
    hline(y,2,W-1," ",nil,shade)
  end
end

-- Smooth scroll transition
local function smoothScroll(lines, dx, dy, delay)
  delay = delay or 0.01
  for step=1,math.max(math.abs(dx),math.abs(dy)) do
    term.scroll(dy>0 and 1 or (dy<0 and -1 or 0))
    sleep(delay)
  end
end

-- Particles
local function particles(dt)
  local th=theme()
  for i=1, math.min(60, math.floor(W*H/80)) do
...
-------------------- BUTTONS / TOUCH -------------------
local buttons={}
local function makeButtons(x1,y1,x2,y2,options) buttons={}; local height=y2-y1+1; local per=#options; if per<1 then return end
  local lineH=math.max(3, math.floor(height/per)); local y=y1
  for i,opt in ipairs(options) do local y2b=math.min(y2,y+lineH-1); table.insert(buttons,{x1,y,x2,y2b,opt.label or "", i, opt.hint, opt.id}); y=y2b+1 end
end
local function drawButtons(th)
  for _,b in ipairs(buttons) do local x1,y1,x2,y2,label,idx=b[1],b[2],b[3],b[4],b[5],b[6]
    drawBox(x1,y1,x2,y2,th.frame,th.bg); local txt=tostring(idx)..") "..label; local mid=math.floor((y1+y2)/2)
    setc(th.btnNum,th.bg); term.setCursorPos(cx(txt),mid); term.write(tostring(idx))
    setc(th.fg,th.bg); term.setCursorPos(cx(txt)+2,mid); term.write(") "..label)
  end
end
local function waitChoice()
  while true do local e,a,b,c = os.pullEvent()
    if e=="monitor_touch" then local mx,my=b,c
    ...
    elseif e=="key" then if a>=keys.one and a<=keys.nine then return (a-keys.one)+1 elseif a==keys.enter or a==keys.numPadEnter then return 1 end end
  end
end
...
local function drawLayout(title,left,right)
  local th=theme()
  hline(1,1,W," ",nil,th.titleBg); hline(2,1,W," ",nil,th.titleBg); writeCentered(1, title or "ENTERTAINMENT STATION", th.titleFg, th.titleBg)
...
local function load(slot)
  ...
  local ok,t=pcall(textutils.unserialize,s); if ok and type(t)=="table" and t.state then
...
local function musicTick()
  if CONFIG.mute or CONFIG.music<=0 or not sfx_ok() then return end
  playSound("minecraft:music.menu", CONFIG.music*CONFIG.master, 1.0)
end

-------------------- ACHIEVEMENTS ----------------------
local ACH={
  firstChoice={name="First Step", desc="Du hast deine erste Entscheidung getroffen."},
  braveTwo   ={name="Bold Soul",  desc="Mut erreicht Stufe 2."},
  lore1      ={name="Archivist",  desc="Erste Lore freigeschaltet."},
...
-------------------- JOURNAL / INVENTORY ---------------
local function journalAdd(line) table.insert(state.journal.summary, line) end
...
local LORE={
  runeShard={name="Runen-Splitter", text="Ein Bruchstueck, das leise vibriert. In Naehe der Halle reagiert es."},
  echoLeaf ={name="Echoblatt",      text="Ein Blatt, dessen Adern ein Muster wie Wellen tragen."},
}
local function addItem(key)
  state.inventory[key]=(state.inventory[key] or 0)+1
  state.journal.loreUnlocked[key]=true
  unlock("lore1")
end
...
local MAP = {
  nodes={
    Gate={x=10,y=8,label="Tor"},
    Court={x=30,y=10,label="Hof"},
    Hall={x=50,y=8,label="Halle"},
    Depth={x=70,y=12,label="Tiefe"},
    Secret={x=40,y=16,label="Geheim"},
  },
  paths={{"Gate","Court"},{"Court","Hall"},{"Hall","Depth"},{"Court","Secret"}}
}
local function drawMap()
  ...
  drawBox(x1,y1,x2,y2,th.frame,th.bg)
  -- paths
  for _,p in ipairs(MAP.paths) do local a=MAP.nodes[p[1]]; local b=MAP.nodes[p[2]]
    local ax=x1+a.x; local ay=y1+a.y; local bx=x1+b.x; local by=y1+b.y
    local dx=bx-ax; local steps=math.max(1,math.abs(dx))
    for i=0,steps do local xx=ax+math.floor(i*dx/steps); local yy=ay+math.floor((by-ay)*i/steps); term.setCursorPos(xx,yy); setc(th.neutral,th.bg); term.write("*") end
  end
  -- nodes
  for k,n in pairs(MAP.nodes) do
    term.setCursorPos(x1+n.x,y1+n.y); setc(th.fg,th.bg); term.write("O")
    term.setCursorPos(x1+n.x+2,y1+n.y); term.write(n.label)
  end
  -- marker
  local cur=MAP.nodes[state.mapLoc] or MAP.nodes.Gate
  term.setCursorPos(x1+cur.x,y1+cur.y); setc(th.accent,th.bg); term.write("@")
  -- back button (touch)
  local backBtn={x1,y2-2,x1+14,y2-0}
  drawBox(backBtn[1],backBtn[2],backBtn[3],backBtn[4],th.frame,th.bg)
  term.setCursorPos(backBtn[1]+2, backBtn[2]+1); setc(th.fg,th.bg); term.write("< Zurueck")
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" then if b>=backBtn[1] and b<=backBtn[3] and c>=backBtn[2] and c<=backBtn[4] then return end end
    if e=="key" and (a==keys.enter or a==keys.back) then return end
  end
end
...
local function drawPortrait(x,y,mood)
  local th=theme(); local art=PORTRAIT[mood] or PORTRAIT.neutral
  for i=1,#art do term.setCursorPos(x,y+i-1); setc(th.fg, th.bg); term.write(art[i]) end
end
local function setMood(m) state.portraitMood=m end

-------------------- MENUS (Theme/Scale w/ Back) -------
local function themeMenu()
  local th=theme()
  local names={}; for k,_ in pairs(THEMES) do table.insert(names,k) end; table.sort(names)
  while true do
    clear(th.bg); drawLayout("Theme auswaehlen","Touch: Auswahl | Zurueck unten","")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    drawBox(x1,y1,x2,y1+4,th.frame,th.bg)
    writeCentered(y1+2,"Aktuell: "..CONFIG.theme, th.fg, th.bg)
    local opts={}; for _,n in ipairs(names) do table.insert(opts,{label=n}) end
    table.insert(opts,{label="< Zurueck"})
    makeButtons(x1,y1+6,x2,y2,opts); drawButtons(th)
    local c=waitChoice()
    if c==#opts then return end
    CONFIG.theme=names[c] or CONFIG.theme; saveConfig(); th=theme()
  end
end

local function scaleMenu()
  local th=theme(); local scales={0,"0.5","1.0","1.5","2.0","2.5","3.0"}
  while true do
    clear(th.bg); drawLayout("Textgroesse","Touch: Auswahl | Zurueck unten","")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    local opts={{label="Auto (empfohlen)"}}
    for i=2,#scales do table.insert(opts,{label="Skalierung "..scales[i]}) end
    table.insert(opts,{label="< Zurueck"})
    makeButtons(x1,y1+2,x2,y2,opts); drawButtons(th)
    local c=waitChoice()
    if c==#opts then return end
    if c==1 then CONFIG.textScale=0; saveConfig(); -- auto
      local targetW,targetH=90,30; local bestS,bestDiff=1.0,1e9
      for s=0.5,3.0,0.5 do mon.setTextScale(s); local w,h=term.getSize(); local d=math.abs(w-targetW)+math.abs(h-targetH); if d<bestDiff then bestDiff,bestS=d,s end end
      mon.setTextScale(bestS)
    else
      local val=tonumber(scales[c]); if val then CONFIG.textScale=val; mon.setTextScale(val); saveConfig() end
    end
    W,H=term.getSize()
  end
end
...
local function settingsMenu()
  local th=theme()
  while true do
    clear(th.bg); drawLayout("Einstellungen","Touch: Regler/Optionen","")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    local opts={
      {label="Theme"}, {label="Textgroesse"},
      {label="Shader: "..(CONFIG.shader and "An" or "Aus")},
      {label=("Master: %.1f"):format(CONFIG.master)},
      {label=("Musik:  %.1f"):format(CONFIG.music)},
      {label=("SFX:    %.1f"):format(CONFIG.sfx)},
      {label=("Mute:   %s"):format(CONFIG.mute and "Ja" or "Nein")},
      {label="< Zurueck"},
    }
    makeButtons(x1,y1+2,x2,y2,opts); drawButtons(th)
    local c=waitChoice()
    if c==1 then themeMenu()
    elseif c==2 then scaleMenu()
    elseif c==3 then CONFIG.shader=not CONFIG.shader; saveConfig()
    elseif c==4 then CONFIG.master=math.max(0, math.min(1, CONFIG.master+ (0.1))); saveConfig()
    elseif c==5 then CONFIG.music =math.max(0, math.min(1, CONFIG.music + (0.1))); saveConfig()
    elseif c==6 then CONFIG.sfx   =math.max(0, math.min(1, CONFIG.sfx   + (0.1))); saveConfig()
    elseif c==7 then CONFIG.mute=not CONFIG.mute; saveConfig()
    else return end
  end
end
...
local function statsScreen()
  ...
  local lines={
    ...
    "Endings: "..tostring((state.endings and (function() local c=0; for _ in pairs(state.endings) do c=c+1 end; return c end)() or 0)),
    "Spielzeit: "..formatTime(played),
  }
  local y=FRAME.titleH+2; for _,ln in ipairs(lines) do term.setCursorPos(4,y); setc(theme().fg, theme().bg); term.write(ln); y=y+2 end
  -- back
  local bx1,by1=4,H-4; drawBox(bx1,by1,bx1+12,by1+2,th.frame,th.bg); term.setCursorPos(bx1+2,by1+1); term.write("< Zurueck")
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" and b>=bx1 and b<=bx1+12 and c>=by1 and c<=by1+2 then return end
    if e=="key" and (a==keys.back or a==keys.enter) then return end
  end
end

local function inventoryScreen()
  local th=theme(); clear(th.bg); drawLayout("Inventar / Lore","Tippe: zurueck","")
  local y=FRAME.titleH+2; term.setCursorPos(4,y); term.write("Gegenstaende:"); y=y+1
  for k,v in pairs(state.inventory) do y=y+1; term.setCursorPos(6,y); term.write( (LORE[k] and LORE[k].name or k)..": x"..v) end
  y=y+2; term.setCursorPos(4,y); term.write("Lore Eintraege:"); y=y+1
  ...
  local bx1,by1=4,H-3; drawBox(bx1,by1,bx1+12,by1+1,th.frame,th.bg); term.setCursorPos(bx1+2,by1); term.write("< Zurueck")
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" and b>=bx1 and b<=bx1+12 and c>=by1 and c<=by1+1 then return end
    if e=="key" and (a==keys.back or a==keys.enter) then return end
  end
end
...
local function journalScreen()
  ...
  local y=FRAME.titleH+2; term.setCursorPos(4,y); term.write("Zusammenfassung:"); y=y+1
  for _,ln in ipairs(state.journal.summary) do y=y+1; term.setCursorPos(6,y); term.write("- "..ln) end
  y=y+2; term.setCursorPos(4,y); term.write("Quests:"); y=y+1
  for id,q in pairs(state.journal.quests) do y=y+1; term.setCursorPos(6,y); term.write((q.done and "[x] " or "[ ] ")..q.text) end
  local bx1,by1=4,H-3; drawBox(bx1,by1,bx1+12,by1+1,th.frame,th.bg); term.setCursorPos(bx1+2,by1); term.write("< Zurueck")
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" and b>=bx1 and b<=bx1+12 and c>=by1 and c<=by1+1 then return end
    if e=="key" and (a==keys.back or a==keys.enter) then return end
...
local function pauseMenu()
  ...
  while true do
    ...
    local c=waitChoice()
    if c==1 then return
    elseif c==2 then journalScreen()
    elseif c==3 then inventoryScreen()
    elseif c==4 then settingsMenu()
    elseif c==5 then save(state.slot)
    elseif c==6 then drawMap()
...
local function askName()
  local th=theme()
  while true do
    clear(th.bg); drawLayout("Wie heisst dein Bote?","Tippe Feld zum Editieren","Enter = OK")
    local x1,y1=10,math.floor(H/2)-2; local x2=x1+40; drawBox(x1,y1,x2,y1+2,th.frame,th.bg)
    term.setCursorPos(x1+2,y1+1); setc(th.fg,th.bg); term.write(state.playerName or "")
    ...
    while true do local e,a,b,c=os.pullEvent()
      ...
      elseif e=="key" and (a==keys.enter or a==keys.numPadEnter) then if #state.playerName>0 then return end end
    end
  end
end
...
local function drawSceneFrame(title, subtitle)
  ...
  drawPortrait(W-9, FRAME.titleH+1, state.portraitMood)
end
...
local function doChoice(prompt, options)
  ...
  -- typewriter
  local width=(x2-1)-(x1+1); local text=prompt
  local function wrap(s,w) local lines={}; s=tostring(s or ""); for p in s:gmatch("([^\n]+)\n?") do
    ...
    end; if #lines==0 then table.insert(lines,"") end; return lines end
  local lines=wrap(text, width)
  local y=y1+1; setc(th.fg,th.bg)
  for _,ln in ipairs(lines) do if y>=y2-9 then break end; term.setCursorPos(x1+2,y); term.write(ln); y=y+1; sleep(0.01) end
  -- options
  ...
  -- wait
  local idx=waitChoice(); return idx
end
...
local function puzzleRunes()
  ...
  local seq={"A","C","B"}; local idx=doChoice("Ordne die Runen: A, B, C. Was ist die Mitte?", { {label="A"}, {label="B"}, {label="C"} })
...
-------------------- SCENES / STORY --------------------
-- dynamic flags affect later options
...
local function introCutscene()
  ...
  particles(0.1); musicTick(); while true do local e=os.pullEvent(); if e=="monitor_touch" or e=="key" then break end end
end

local function scene_intro()
  drawSceneFrame("Die Flusternde Ruine", "Willkommen, "..(state.playerName or ""))
  setMood("neutral")
  local idx=doChoice("Du stehst vor einem alten Tor. Die Luft ist kuehl. Was tust du, "..state.playerName.."?", {
    {label="Mutig eintreten"},
    {label="Erst den Hof erkunden"},
    {label="Mit der Steinfigur sprechen"},
  })
...
local function scene_courtyard()
  ...
  local opts={
    {label="Inschriften entschluesseln"},
    {label="Dem Summen zur Halle folgen"},
    optionIf(state.empathy>=2, {label="Einem Kind helfen (Nebenfigur)"})
  }
...
local function scene_hall()
  ...
  local opts={
    {label="Mit der Tuer sprechen"},
    optionIf(state.flags.gotRune, {label="Runen einsetzen"}),
    optionIf(state.courage>=2, {label="Dunklen Gang erforschen"}),
  }
  local filtered={}; for _,o in ipairs(opts) do if o then table.insert(filtered,o) end end
...
local function scene_depth()
  ...
  local idx=doChoice("In der Tiefe fuehlst du eine Gegenwart.", {
    {label="Rufe laut (Mut)"},
    {label="Horche geduldig (Verstand)"},
    {label="Sprich sanft (Mitgefuehl)"},
  })
  if idx==1 then statUp("courage",1); setMood("brave")
  elseif idx==2 then statUp("wisdom",1); setMood("neutral")
  else statUp("empathy",1); setMood("happy")
  end
...
local function scene_secret()
  ...
  if idx==1 then journalAdd("Du betrittst den geheimen Pfad."); unlock("secret"); addXP(10); return "ending_secret" else return "final" end
end
...
local function doEnding(kind)
  ...
  else title="Pfad des Mutes"; msgLeft="Du schreitest voran, trotz Dunkel."; msgRight="Mut traegt dich."
  end
...
local function scene_ending_secret()
  ...
  local idx=doChoice("Du findest eine uralte Inschrift. Willst du den Schwur erneuern und die Ruine bewahren?", { {label="Ja, Schwur erneuern"}, {label="Nein, weiterziehen"} })
  if idx==1 then journalAdd("Geheimer Schwur geleistet."); return "credits" else return "credits" end
end
...
local scenes={
  ...
  hall=scene_hall,
  riddle1=scene_riddle1,
  riddle2=scene_riddle2,
  depth=scene_depth,
  final=scene_final,
  secret=scene_secret,
  ending_secret=scene_ending_secret,
  credits=credits,
}
...
local function autosaveRecap()
  ...
  save(state.slot)
  -- kurze Rueckblende beim naechsten Laden: nimm die letzten 2 Journal-Zeilen
  local n=#state.journal.summary
  local recap=""
...
local function showStatsSummary(x,y)
  ...
  term.setCursorPos(x,y); setc(th.fg,th.bg); term.write("Spielzeit: "..formatTime(played))
...
local function mainMenu()
  ...
  while true do
    ...
    local c=waitChoice()
    if c==1 then if fs.exists(savePath(state.slot)) and load(state.slot) then
        if state.lastRecap and #state.lastRecap>0 then clear(th.bg); drawLayout("Rueckblende", state.lastRecap, ""); sleep(1.2) end
        return "resume"
      else return "new" end
    elseif c==2 then return "new"
    elseif c==3 then
      -- slot picker
      clear(th.bg); drawLayout("Save Slots","Touch: Slot | Zurueck unten","")
      local o={{label="Slot 1"},{label="Slot 2"},{label="Slot 3"},{label="Slot loeschen (aktuell)"},{label="< Zurueck"}}
      makeButtons(5,FRAME.titleH+4,W-5,H-FRAME.statusH-1,o); drawButtons(th)
      local s=waitChoice(); if s>=1 and s<=3 then state.slot=s elseif s==4 then deleteSave(state.slot) end
    elseif c==4 then settingsMenu()
    elseif c==5 then statsScreen()
    elseif c==6 then quoteExport()
    else return "quit"
    end
  end
end
...
local function game()
  ...
  while true do
    ...
    while true do
      ...
      elseif e=="timer" and a==t then break
      end
    end
  end
end
...
while true do
  ...
  if act=="new" then
    state = {
      slot=state.slot, playerName="", scene="intro", courage=0, wisdom=0, empathy=0, level=1, xp=0,
      flags={}, decisions=0, endings={}, timeStart=os.epoch("utc"), playTime=0, mapLoc="Gate",
      journal={summary={}, quests={}, loreUnlocked={}, notes={}}, inventory={}, achievements={},
      autosave=true, lastRecap="", portraitMood="neutral", portraitTick=0
    }
    save(state.slot)
