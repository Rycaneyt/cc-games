-- fake_admin.lua
-- Dual Output (Terminal + Monitor), Touch in allen Menüs, On-Screen-Keyboard, Font +/-.
-- Für CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "König Fabio"
local VERSION = "FabIO-AdminOS v2.0"
local INITIAL_SCALE = 0.5     -- Start-Schriftgröße Monitor (0.5 = klein/hochauflösend)
...
-- Helper: sichere Methode aufrufen, falls vorhanden
local function try(t, m, ...)
  if t and t[m] then return t[m](...) end
end

-- Multi-Terminal API mit allen erwarteten Methoden
local function makeMultiTerm(t1, t2)
  ...
  local function each(fn)
    for _,t in ipairs(terms) do fn(t) end
  end
  ...
  function api.isColour()              -- britische Schreibweise, von CC erwartet
    if terms[1].isColour then return terms[1].isColour()
    elseif terms[1].isColor then return terms[1].isColor()
...
local function line(y, ch)
  term.setCursorPos(1,y); term.write(string.rep(ch or "─", w))
end
local function clearAndHeader(withButtons)
  term.clear(); term.setCursorPos(1,1)
  term.setTextColor(colors.white); centerText(1, VERSION)
  term.setTextColor(colors.gray);  line(2, "─")
  term.setTextColor(colors.white)
  if withButtons then
    -- Font +/- Buttons oben rechts
    local label = "[ - ] [ + ]"
    local x = w - #label
    term.setCursorPos(x,1); term.write(label)
  end
end
...
-- Klickflächen-Manager (für Touch)
local touchAreas = {}  -- { {x1,y1,x2,y2, id, handler=function() end}, ... }
...
-- registriert globale +/− Buttons (oben rechts)
local function registerFontButtons()
  local label = "[ - ] [ + ]"
  local x = w - #label
  -- [ - ]
  addTouchArea(x+2, 1, x+4, 1, "font_minus", function()
    changeScale(-SCALE_STEP)
  end)
  -- [ + ]
  addTouchArea(x+8, 1, x+10, 1, "font_plus", function()
    changeScale(SCALE_STEP)
  end)
end
...
-- Tasten-ReadLine (Touch: nichts; Tastatur: normal)
local function waitKeyOrTouch()
  while true do
    local ev, p1, p2, p3 = os.pullEvent()
    if ev == "char" or ev == "key" then return ev, p1 end
    if ev == "monitor_touch" and mon and p1 == peripheral.getName(mon) then
      return ev, p2, p3
    end
    if ev == "term_resize" then resizeCache() end
  end
end

-- Menü-Auswahl: Zahl (Tastatur) oder Zeilen-Touch
local function menuSelect(yFirst, count, acceptsChars)
  -- yFirst: erste Menüzeile (wo Option 1 steht), count: Anzahl Einträge
  while true do
    local ev, a, b = waitKeyOrTouch()
    if ev == "char" and acceptsChars then
      return a
    elseif ev == "char" and not acceptsChars then
    ...
    elseif ev == "monitor_touch" then
      local tx,ty = a,b
      local id = handleTouch(tx,ty)
      if id == "font_minus" or id == "font_plus" then
        -- schon behandelt
      else
        -- Klick auf Menüzeile?
        for i=1,count do
          if ty == yFirst + (i-1) then return tostring(i) end
        end
      end
    end
  end
...
-- On-Screen-Keyboard (QWERTZ) + Tastatur: für Login
local KB_ROWS = {
  "1234567890",
  "qwertzuiop",
  "asdfghjkl",
  "< yxcvbnm  >", -- < = Backspace, two spaces include Space area, > = Enter
}
local function drawKeyboard(yStart)
  ...
  for r,row in ipairs(KB_ROWS) do
    ...
    -- zeichnen + Touchflächen eintragen
    local x = xStart
    for i = 1, #row do
      local ch = row:sub(i,i)
      local disp = ch
      if ch == "<" then disp = "[Bksp]" end
      if ch == " " then disp = " " end
      if ch == ">" then disp = "[Enter]" end
      term.setCursorPos(x, yStart + r - 1); term.write(disp)
      -- Toucharea definieren
      addTouchArea(x, yStart + r - 1, x + (#disp-1), yStart + r - 1, "kb_"..r.."_"..i, function()
        -- nothing here; Auswertung in readLineTouch
      end)
      x = x + #disp + 1
    end
  end
...
local function readLineTouch(promptX, promptY, masked)
  ...
  local kbY = promptY + 2
  drawKeyboard(kbY); registerFontButtons()
  while true do
    ...
    elseif ev == "monitor_touch" then
      ...
      if id == "font_minus" or id == "font_plus" then
        ...
        local key = keyboardHitToChar(tx,ty,kbY)
    ...
    elseif ev == "term_resize" then
      resizeCache(); clearAndHeader(true)
      term.setCursorPos(1, promptY-2); print("") -- nur, um Layout neu zu zeichnen
      drawKeyboard(kbY); registerFontButtons()
    end
  end
end
...
local function explosionAnimation()
  clearTouchAreas(); clearAndHeader(true); registerFontButtons()
  centerText(4, "WARNUNG: Selbstzerstörungs-Protokoll wird initialisiert")
  term.setTextColor(colors.red)
  for n, note in ipairs({"5","4","3","2","1"}) do
...
local function hackMojangAnimation()
  ...
  local barY = 6
  term.setCursorPos(4, barY); term.write("[")
  term.setCursorPos(w-3, barY); term.write("]")
  for i = 1, (w - 10) do
  ...
  term.setTextColor(colors.white); centerText(8, "Zugriff auf Mojang: ERLAUBT (fake).")
...
local function fabioDance()
  ...
  local frames = { [[\o/   .  _   .   \o/]], [[ _\o  . ( )  .  _\o_]], [[  /|  . / \  .   /| ]], [[ \o   .  ∆   .   \o/]] }
...
local function sillyOptions()
  ...
  term.setCursorPos(2, baseY + #items + 2); term.write("Tippe/Enter zum Zurückkehren.")
  -- warte auf Eingabe/Tap
  while true do
    local ev, a, b = waitKeyOrTouch()
    if ev == "key" and a == keys.enter then return end
    if ev == "monitor_touch" then
      local id = handleTouch(a,b); if id then return end
    end
    if ev == "char" and (a == '+' or a == '-') then
      changeScale(a=='+' and SCALE_STEP or -SCALE_STEP)
      clearTouchAreas(); sillyOptions(); return
    end
  end
end
...
local function fakeRestart()
  ...
  for i = 1,3 do
    centerText(6, "Neustart in " .. (4 - i) .. "..."); safePlayNote("C","bass",0.8); sleep(0.8)
  end
  term.clear(); centerText(6, "Start erfolgreich. Alles wirklich genau so wie vorher.")
  addLog("[INFO] System 'neustart' ausgeführt (fake).", false); sleep(1)
end
...
local function corruptFiles()
  ...
  addLog("[WARN] Scan nach peinlichen Dateien: 0/0 (fehlerhaft).", true); sleep(1.2)
end

-------------------- Logs Ansicht (Scroll + Touch) --------------------
local function showLogs()
  ...
  while true do
    ...
    for i = 1, maxLines do
      ...
      if logs[idx] then
        if logs[idx]:find("WARN") or logs[idx]:find("FEHLER") then term.setTextColor(colors.red)
        elseif logs[idx]:find("FUN") or logs[idx]:find("CAT") then term.setTextColor(colors.magenta)
    ...
    term.setCursorPos(2, baseY + maxLines + 1); term.write("↑/↓ scrollen, Enter zurück")
    term.setTextColor(colors.white)
    -- Touch Scrollflächen
...
local function mainMenu()
  while true do
    resizeCache(); clearTouchAreas(); clearAndHeader(true); registerFontButtons()
    centerText(4, "== Super-Geheimes Admin-Interface ==")
    local baseY = 6
    local entries = {
      "Server Selbstzerstörung aktivieren",
      "Hacke das Mojang-Netzwerk",
      "Aktiviere " .. USERNAME_DISPLAY .. "'s geheime Tanzroutine",
      "Zeige Logs",
    ...
    term.write("Wähle (Zahl) oder tippe Option. [+/-] Schriftgröße")

    local sel = menuSelect(baseY, #entries, false)
    addLog("[UI] Auswahl: " .. tostring(sel), false)
    ...
    elseif sel == "8" or sel == "0" then
      ...
      clearAndHeader(true); centerText(6, "Auf Wiedersehen, " .. USERNAME_DISPLAY .. "!")
      safePlayNote("C","pling",0.8); sleep(1); term.clear(); return
    end
  end
end

-------------------- Login (mit On-Screen-Keyboard) --------------------
local function login()
  resizeCache(); clearTouchAreas(); clearAndHeader(true); registerFontButtons()
  term.setTextColor(colors.lightBlue); centerText(3, "== SECURE ADMIN ACCESS ==")
  term.setTextColor(colors.white)
  centerText(5, "Bitte melden Sie sich an. Nur Admins dürfen fortfahren.")

  term.setCursorPos(4,7); term.write("Username: ")
  local user = readLineTouch(14,7,false)
  term.setCursorPos(4,8); term.write("Password: ")
  local pass = readLineTouch(14,8,true)

  loadingDots(4 + 18, 10, 1.4)
  term.setCursorPos(4,10); term.setTextColor(colors.green)
...
local function start()
  ...
  while true do
    ...
    if ev == "monitor_touch" then
      ...
      if not id then break end  -- irgendwo tippen = weiter
    elseif ev == "char" then
      if a == '+' then changeScale(SCALE_STEP)
      elseif a == '-' then changeScale(-SCALE_STEP)
      else break end
