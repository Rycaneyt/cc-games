...
-- ASCII-only. Zweiphasiger Login: erst Status, dann Eingabe. Auto-Weiter oder per Klick/Enter.
-- CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "Koenig Fabio"
local VERSION = "FabIO-AdminOS v2.4"
local INITIAL_SCALE = 1.5          -- nur Monitor
local MIN_SCALE, MAX_SCALE = 0.5, 5.0
local SCALE_STEP = 0.5
-- Login-Timing
local LOGIN_STATUS_AUTOSECONDS = 5 -- nach X s automatisch weiter
...
local function makeMultiTerm(t1, t2)
  ...
  function api.getSize() local w1,h1=terms[1].getSize(); if terms[2] then local w2,h2=terms[2].getSize(); return math.min(w1,w2),math.min(h1,h2) end; return w1,h1 end
...
-------------------- Systemmeldungen (dezente Tonalitaet) --------------------
...
local function drawLoginStatusPanel(topY)
  ...
  term.setCursorPos(col2,topY+3); term.write("IP:          "..fakeIP)
  ...
  term.setCursorPos(col2,topY+8); term.write("2FA:         optional")
  term.setCursorPos(col2,topY+9); term.write("Skew:        0."..math.random(1,9).."s")
end

-------------------- Aktionen (serioes gehalten) --------------------
local function explosionAnimation()
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Sicherheitsprotokoll 7B: Vorbereitung ...")
  term.setTextColor(colors.red)
  for _,n in ipairs({"5","4","3","2","1"}) do centerText(6,n); safePlayNote("C","bass",1); sleep(0.65) end
  term.setTextColor(colors.magenta); centerText(8,"Abbruch durch Richtlinie. Kein Vorgang ausgefuehrt.")
  safePlayNote("C","pling",1); sleep(1.6); term.setTextColor(colors.white)
  addLog("[ACTION] Kritischer Vorgang angefragt; durch Richtlinie blockiert.")
end
...
local function hackMojangAnimation()
  ...
  addLog("[SEC] Externe Diagnose ausgefuehrt (Daten anonym)."); sleep(1.2)
end
...
local function fabioDance()
  ...
  addLog("[INFO] Interaktives Modultest-Pattern gespielt."); term.setTextColor(colors.white); sleep(1.2)
end
...
local function sillyOptions()
  ...
  local baseY=6
  local items={"Richtlinie 'Keks-Regel' anzeigen","Ego-Telemetrie kalibrieren","Visual Test: Dancefloor (unsichtbar)","Automatisches Feedback senden"}
  for i,v in ipairs(items) do term.setCursorPos(4,baseY+i-1); term.write(i..") "..v); addTouchArea(1,baseY+i-1,w,baseY+i-1,"silly_"..i,function() end) end
  addLog("[INFO] Dienstprogramme geoeffnet.")
  term.setCursorPos(2, baseY + #items + 2); term.write("Tippe/Enter zum Zurueckkehren.")
  while true do local ev,a,b=waitKeyOrTouch(); if ev=="key" and a==keys.enter then return end; if ev=="monitor_touch" then local id=handleTouch(a,b); if id then return end end; if ev=="char" and (a=="+" or a=="-") then changeScale(a=="+" and SCALE_STEP or -SCALE_STEP) end; if ev=="term_resize" then clearTouchAreas(); return sillyOptions() end end
end
...
local function fakeRestart()
  ...
  term.clear(); centerText(6,"Start abgeschlossen."); addLog("[INFO] Geplanter Neustart simuliert."); sleep(1.4)
end

local function corruptFiles()
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Dateisystem-Scan ...")
  for i=1,w-10 do term.setCursorPos(5+i,8); term.write(string.char(33+(i%60))); sleep(0.006) end
  centerText(10,"Scan: keine relevanten Artefakte."); addLog("[INFO] Dateisystem-Scan ohne Befund."); sleep(1.3)
end
...
local function showLogs()
  ...
  while true do
    ...
    local maxLines=h-9; local baseY=6
    for i=1,maxLines do local idx=pos+i-1; term.setCursorPos(2,baseY+i-1); term.clearLine(); if logs[idx] then term.setTextColor(colors.white); local text=logs[idx]; if #text>w-4 then text=text:sub(1,w-7).."..." end; term.write(text) end end
    term.setTextColor(colors.gray); term.setCursorPos(2,baseY+maxLines+1); term.write("Pfeile scrollen, Enter zurueck, +/- Schriftgroesse"); term.setTextColor(colors.white)
    ...
    local ev,a,b=waitKeyOrTouch()
    if ev=="key" then if a==keys.up then pos=math.max(1,pos-1) elseif a==keys.down then pos=math.min(math.max(1,#logs-maxLines+1),pos+1) elseif a==keys.enter then return
      elseif a==keys.minus or a==keys.numPadSubtract then changeScale(-SCALE_STEP) elseif a==keys.equals or a==keys.numPadAdd then changeScale(SCALE_STEP) end
    elseif ev=="char" then if a=="+" then changeScale(SCALE_STEP) elseif a=="-" then changeScale(-SCALE_STEP) end
    elseif ev=="monitor_touch" then handleTouch(a,b)
    end
  end
end
...
local function mainMenu()
  while true do
    ...
    local baseY=6
    local entries={"Sicherheitsprotokoll 7B testen","Remote-Diagnose","Interaktives Modul (Test)","Logs anzeigen","Dienstprogramme","Geplanter Neustart","Dateisystem-Scan","Beenden"}
    for i,v in ipairs(entries) do term.setCursorPos(4,baseY+i-1); term.write(i%8==0 and "0) "..v or (i..") "..v)); addTouchArea(1,baseY+i-1,w,baseY+i-1,"menu_"..i,function() end) end
    term.setCursorPos(4,baseY+#entries+2); term.write("Zahl eingeben oder tippen. [+/-] Schriftgroesse")
    local sel=menuSelect(baseY,#entries); addLog("[UI] Auswahl: "..tostring(sel))
    ...
    elseif sel=="8" or sel=="0" then addLog("[INFO] Sitzung beendet."); clearAndHeader(true); drawScaleLabelAndButtons(); centerText(6,"Abmeldung ..."); safePlayNote("C","pling",0.7); sleep(1.5); term.clear(); return end
  end
end

-------------------- Login (2-Phasen: Intro/Status -> Formular) --------------------
...
-- Phase 1: Intro/Status (blockiert Eingabeform)
local function loginIntro()
  ...
  banner(4)
  drawLoginStatusPanel(8)

  local hint = "Tippen oder Enter, automatisch in "..tostring(LOGIN_STATUS_AUTOSECONDS).." s ..."
  centerText(h-2, hint)
  -- Warte auf Enter/Touch oder Timer
  local timerId = os.startTimer(LOGIN_STATUS_AUTOSECONDS)
  while true do
    local ev,a,b = os.pullEvent()
    if ev=="key" and a==keys.enter then break end
    if ev=="char" then if a=="+" then changeScale(SCALE_STEP) elseif a=="-" then changeScale(-SCALE_STEP) end end
    if ev=="monitor_touch" and mon and a==peripheral.getName(mon) then
      local tx,ty=b,select(4,os.pullEventRaw()) -- Not used safely; safer: re-get via parameters
    end
    if ev=="monitor_touch" then
      local id = handleTouch(a,b); if id~="font_minus" and id~="font_plus" then break end
    elseif ev=="timer" and a==timerId then break
    elseif ev=="term_resize" then
      -- redraw
      resizeCache(); clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
      term.setTextColor(colors.lightBlue); centerText(3,"== AUTHENTICATION REQUIRED =="); term.setTextColor(colors.white)
      banner(4); drawLoginStatusPanel(8); centerText(h-2, hint)
    end
  end
end

-- Phase 2: Formular mit Keyboard
local function loginForm()
  ...
  -- Checks mit Pausen
  local checkY=yForm+3
  term.setCursorPos(4,checkY+0); term.write("Pruefe Zeitsynchronisation"); loadingDots(30,checkY+0,LOGIN_STEP_DELAY); sleep(0.2)
  term.setCursorPos(4,checkY+1); term.write("Lade ACL");                  loadingDots(30,checkY+1,LOGIN_STEP_DELAY); sleep(0.2)
  term.setCursorPos(4,checkY+2); term.write("Validiere Token");           loadingDots(30,checkY+2,LOGIN_STEP_DELAY); sleep(0.2)
  term.setCursorPos(4,checkY+3); term.write("Initialisiere Audit-Stream");loadingDots(30,checkY+3,LOGIN_STEP_DELAY); sleep(0.2)
  term.setCursorPos(4,checkY+4); term.write("Kerberos-Skew: 0."..math.random(1,9).."s"); sleep(LOGIN_STEP_DELAY)

  term.setCursorPos(4,checkY+6); term.setTextColor(colors.green); print("Zugriff gewaehrt. Willkommen, "..USERNAME_DISPLAY.."."); term.setTextColor(colors.white)
  addLog("[AUTH] Anmeldung von '"..tostring(user).."' erfolgreich.")
  sleep(POST_LOGIN_DWELL)
end

local function login()
  loginIntro()   -- Status zuerst (blockiert Keyboard), Auto-weiter oder Enter/Touch
  loginForm()    -- danach sauberes Formular mit Tastatur
end
