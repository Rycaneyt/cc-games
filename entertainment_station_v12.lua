-- entertainment_v3.lua  (v3.5 – keyboard scale/toggle, playtime timer toggle/reset)
...
local CONFIG={
  ...
  touchKeyboardEnabled=true,         -- <— neues Setting
  kbScale=1.0                        -- <— 0.6 .. 1.6 (skaliert Tasten)
}
...
local state={
  ...
  _playTimerOn=false, _playTimerT0=0,
  playTimerEnabled=true,             -- <— Timer global an/aus
  mapLoc="Gate",
...
local function save(slot)
  playTimerStop()  -- vor dem Schreiben konsolidieren
...
local function timerMenu()
  ...
  while true do
    ...
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    local opts={
      {label="Timer: "..(state.playTimerEnabled and "An" or "Aus")},
      {label="Timer zuruecksetzen"},
      {label="< Zurueck"},
    }
    makeButtons(x1,y1+2,x2,y2-4,opts); drawButtons(th)
    local backRect=drawBackButton(th,x1,y2)
    ...
    if e=="monitor_touch" then
      ...
      for idx,bt in ipairs(buttons) do if mx>=bt[1] and mx<=bt[3] and my>=bt[2] and my<=bt[4] then
        if idx==1 then
          if state.playTimerEnabled then playTimerStop(); state.playTimerEnabled=false else state.playTimerEnabled=true end
      ...
      for idx,bt in ipairs(buttons) do if mx>=bt[1] and mx<=bt[3] and my>=bt[2] and my<=bt[4] then
        ...
        elseif idx==4 then
          ...
          while true do
            ...
            local backRect2=drawBackButton(th,ax1,ay2)
...
local function touchKeyboard(prompt, initial)
  ...
  while true do
    ...
    -- Dynamische Tastenbreite/hoehe mit Scale + Auto-Fit
    local spacing=1
    ...
    for r=1,#KB_ROWS do
      ...
      for c=1,#row do
        local label=row[c]
        -- letzte Reihe hat 9 Keys -> neu berechnen fuer die Reihe
        local colsR=#row
...
local function askName()
  if CONFIG.touchKeyboardEnabled then
    ...
    -- Fallback: einfache Text-Eingabe mit Tastatur
    local th=theme(); clear(th.bg); drawLayout("Name eingeben (Tastatur)","Tippe Enter wenn fertig","")
    ...
    while true do
      ...
      elseif e=="key" and (a==keys.enter or a==keys.numPadEnter) then if #state.playerName>0 then break end
...
local function drawSceneFrame(title,subtitle)
  ...
  local Wp,Hp=term.getSize(); -- Portrait bleibt rechts
  local px=Wp-9; if px>5 then
    local PORTRAIT={neutral={"  /\\ "," (..)","  || "}, happy={"  /\\ "," (^^)","  \\/ "}, sad={"  /\\ "," (..)","  -- "}, brave={" /\\/"," (><)"," /  \\"}}
    local art=PORTRAIT[state.portraitMood] or PORTRAIT.neutral
    for i=1,#art do term.setCursorPos(px,FRAME.titleH+1+i-1); setc(theme().fg,theme().bg); term.write(art[i]) end
  end
end
...
local function doEnding(kind)
  ...
  local t0=os.clock(); while os.clock()-t0<1.2 do local x=math.random(1,W); local y=math.random(1,H); setc(nil, ({th.good,th.neutral,th.accent,th.bad})[math.random(4)]); term.setCursorPos(x,y); term.write(" ") end
...
local function statsScreen()
  local th=theme()
  local live=state.playTime + (state._playTimerOn and (_now()-state._playTimerT0) or 0)
  scrollTextPanel("Statistik", {
    ...
    "Spielzeit (Story): "..formatTime(live),
...
local function mainMenu()
  local th=theme()
  playTimerStop() -- im Hauptmenue nie zaehlen
...
local function game()
  ...
  introCutscene()
  playTimerStart()  -- ab hier zählt die Story-Zeit (nur wenn erlaubt)
  while true do
    ...
    while true do
      ...
      elseif e=="key" and (a==keys.p) then playTimerStop(); 
        -- Pause-Menue (kompakt hier, mit Zurueck)
