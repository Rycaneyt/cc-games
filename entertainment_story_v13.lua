-- entertainment_v3.lua  (v3.6 – back from name input, live scale in keyboard, keyboard settings UI fix)
...
local CONFIG={
  ...
  touchKeyboardEnabled=true,     -- Toggle fuer Touchkeyboard
  kbScale=1.0                    -- 0.6 .. 1.6
}
...
-- Keyboard-Settings (Toggle + Scale, kein "< Zurueck" Eintrag)
      ...
      for idx,bt in ipairs(buttons) do if mx>=bt[1] and mx<=bt[3] and my>=bt[2] and my<=bt[4] then
        ...
        elseif idx==4 then
          ...
          while true do
            ...
            drawBox(muteBtn[1],muteBtn[2],muteBtn[3],muteBtn[4],th.frame,th.bg); term.setCursorPos(muteBtn[1]+2,mutebtn and mutebtn[2]+1 or ay1+3); term.setCursorPos(muteBtn[1]+2,ay1+3); term.write("Mute: "..(CONFIG.mute and "Ja" or "Nein"))
...
local function touchKeyboard(prompt, initial)
  ...
  while true do
    ...
    -- Dynamische Tastenbreite/hoehe (Auto-Fit + Scale)
    local spacing=1
    ...
    -- Zurueck-Button (bricht Eingabe ab -> zurueck ins Hauptmenue)
...
local function game()
  ...
  playTimerStart()
  while true do
    local handler=( {intro=scene_intro,courtyard=scene_courtyard,hall=scene_hall,riddle1=scene_riddle1,riddle2=scene_riddle2,depth=scene_depth,final=scene_final,secret=scene_secret,ending_secret=scene_ending_secret,credits=function() return "menu" end} )[state.scene] or scene_intro
