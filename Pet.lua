-- pet.lua — Virtual Pet (Touch-first, Inventory, Shop, Quests, Name, Sprite+Blink)
...
local state = load() or {
  ...
  inventory = { Ball=0, Snack=0, Badeset=0, EnergyDrink=0 },
}
...
-------------------- PET SPRITES & DRAW --------------------
local petBlink = false
local petTick  = 0

local function cell(x, y, w, h, bg)
  setBG(bg); for yy=y, y+h-1 do term.setCursorPos(x, yy); term.write((" "):rep(w)) end; setBG(colors.black)
end
local function bodyColor(state, sleeping)
  if sleeping then return colors.blue
  elseif state.health < 30 then return colors.green
  elseif state.mood < 40 then return colors.gray
  else return colors.lightBlue end
end
local function petFaceFor(state, sleeping)
  if sleeping then return {eyes="--", mouth="_"} end
  if state.health < 30 then return {eyes="x x", mouth="_"} end
  if state.mood >= 70 then return {eyes="^ ^", mouth="u"} end
  if state.mood >= 40 then return {eyes="- -", mouth="-"} end
  return {eyes="o o", mouth="_"}
end
local function drawPetAt(px, py, sleeping)
  local body = bodyColor(state, sleeping)
  cell(px+2,  py+0, 12, 1, body)
  cell(px+1,  py+1, 14, 1, body)
  cell(px+0,  py+2, 16, 3, body)
  cell(px+2,  py+5, 12, 1, body)

  local faceTbl = petFaceFor(state, sleeping)
  local eyes = (petBlink and not sleeping) and "- -" or faceTbl.eyes
  setBG(body); setFG(colors.black)
  term.setCursorPos(px+4, py+2); term.write(eyes)
  term.setCursorPos(px+7, py+4); term.write(faceTbl.mouth)
  setBG(colors.black); setFG(colors.white)
end
...
local function drawSoftbar(left, mid, right)
  ...
  center(y, lw..mw..rw, colors.lightGray)
end
local function drawShop(sel)
  clr()
  center(1,"SHOP",colors.yellow)
  center(2,"Coins: "..state.coins,colors.cyan)
  center(3,"Tippe [▲]/[▼] oder Artikel. Unten: [Kaufen]/[Zurück]",colors.lightGray)
...
local function drawInv(sel)
  ...
  center(2,"Tippe Item. Unten: [Benutzen]/[Zurück]",colors.lightGray)
  local y=4
  for i,it in ipairs(itemDefs) do
...
local function drawQuests()
  ...
  -- Softbar: [Zurück] [Neu würfeln (-5c)]
  local y=H(); setBG(colors.black); setFG(colors.lightGray)
  local left="[ Zurück ]  "; local mid="[ Neu würfeln (-5c) ]"
  center(y, left..mid, colors.lightGray)
end
...
local function draw()
  ...
  -- Pet am ENDE zeichnen (sichtbar über allem)
  do
    local petW, petH = 16, 6
    local maxX, maxY = W(), H()
    local px = math.floor((maxX - petW)/2) + 1
    if px < 1 then px = 1 end
    local py = math.floor(maxY/2) - math.floor(petH/2)
    if py < 5 then py = 5 end
    if py + petH > maxY - 4 then py = (maxY - 4) - petH end
    if py < 3 then py = 3 end
    local sleeping = (state.energy <= 15)
    drawPetAt(px, py, sleeping)
  end
end
...
while true do
  ...
  elseif e=="monitor_touch" or e=="mouse_click" then
    ...
    if btn then
      ...
      elseif btn.id=="inv"    then invLoop(); draw()
      elseif btn.id=="name"   then changeName(); draw()
      else
        local f=actions[btn.id]; if f then f(); healthRecalc(); moodRecalc(); questCheckRewards(); draw() end
      end
    end
  elseif e=="key" then
