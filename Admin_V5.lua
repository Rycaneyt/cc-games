...
-- ASCII-only. Seriöseres Auftreten, gestaffelte Login-Checks, laengere Lesepausen.
-- CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "Koenig Fabio"
local VERSION = "FabIO-AdminOS v2.3"
...
-- Delays (feintunen nach Geschmack)
local LOGIN_STEP_DELAY = 0.7     -- Zeit pro Checkzeile beim Login
local POST_LOGIN_DWELL  = 2.8     -- Ruhezeit nach "Zugriff gewaehrt"
-------------------------------------------------------
...
local function drawScaleLabelAndButtons()
  ...
  local minusIdx = SCALE_LABEL:find("%-")
...
-------------------- AI Assistent (dezente Tonalitaet) --------------------
...
local function drawLoginStatusPanel(topY)
  ...
  term.setCursorPos(col1, topY+7); term.write("Assets gezaehlt:  " .. math.random(120,980))
  ...
  term.setCursorPos(col2, topY+9); term.write("Kerberos-Skew:    0."..math.random(1,9).."s")
end

-------------------- Aktionen (gleich, leicht dezenter Text) --------------------
local function explosionAnimation()
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4, "Sicherheitsprotokoll 7B: Vorbereitung ...")
  term.setTextColor(colors.red)
  for _,note in ipairs({"5","4","3","2","1"}) do
    centerText(6, note); safePlayNote("C","bass",1); sleep(0.65)
  end
...
local function hackMojangAnimation()
  ...
  addLog("[SEC] Externe Diagnose ausgefuehrt (Daten anonym).")
  sleep(1.2)
end
...
local function fabioDance()
  ...
  for r=1,6 do for i,fr in ipairs(frames) do
    ...
    safePlayNote("C","harp",0.4 + 0.04*i); sleep(0.25) end end
  term.setTextColor(colors.cyan); centerText(12, "Modultest beendet.")
  addLog("[INFO] Interaktives Modultest-Pattern gespielt.")
  term.setTextColor(colors.white); sleep(1.2)
end
...
local function sillyOptions()
  ...
  local items = {
    ...
    "Visual Test: Dancefloor (unsichtbar)",
...
local function fakeRestart()
  ...
  term.clear(); centerText(6, "Start abgeschlossen.")
  addLog("[INFO] Geplanter Neustart simuliert."); sleep(1.4)
end
...
local function corruptFiles()
  ...
  centerText(10, "Scan: keine relevanten Artefakte.")
  addLog("[INFO] Dateisystem-Scan ohne Befund."); sleep(1.3)
end
...
-------------------- Login (seriöser, mit Checks + Pausen) --------------------
local function banner(y)
  term.setCursorPos(3,y);   term.write("ACHTUNG: Zugriff nur fuer autorisierte Administratoren.")
  term.setCursorPos(3,y+1); term.write("Alle Aktionen werden protokolliert. Unautorisierte Zugriffe")
  term.setCursorPos(3,y+2); term.write("koennen disziplinaere Massnahmen nach sich ziehen.")
end

local function login()
  resizeCache(); clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  term.setTextColor(colors.lightBlue); centerText(3, "== AUTHENTICATION REQUIRED ==")
  term.setTextColor(colors.white)
  banner(4)
  ...
  -- gestaffelte, glaubwuerdige Checks mit Pausen
  ...
  -- bewusste Lesepause, damit es ruhiger und echter wirkt
  sleep(POST_LOGIN_DWELL)

  -- dezente Systemmeldungen
  aiSpeak({
    "Sitzungstoken erstellt.",
    "Richtlinien geladen.",
    "Bereit."
  }, 0.75)
