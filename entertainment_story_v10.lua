-- entertainment_v3.lua  (v3.3 – Theme-Fix + Touch-Scroll)
...
-- THEME FIXES: Sunset = orange; Neon vorhanden & grell; plus weitere
local THEMES={
...
-------------------- SCROLL HELPERS --------------------
-- Up/Down-Buttons am rechten Rand eines Bereichs
...
local function themeMenu()
  ...
  local offset=0   -- sichtbarer Startindex (0-basiert)
  local visibleRows=math.max(1, math.floor((listBottom-listTop+1)/3)) -- mit Button-Höhe ~3

  while true do
    th=theme(); clear(th.bg); drawLayout("Theme auswaehlen","Tippe Theme oder Pfeile | < Zurueck","")
    ...
    -- sichtbare Liste bauen
    local opts={}
    for i=1,visibleRows do
      local idx=offset+i
      if names[idx] then table.insert(opts,{label=names[idx]}) end
    end
    makeButtons(x1,listTop,x2-6,listBottom,opts); drawButtons(th)

    -- Scroll-Buttons & Back
    local upBtn,downBtn=drawScrollButtons(th,x2,listTop,listBottom)
    backRect=drawBackButton(th,x1,y2)

    -- Event-Loop
    local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" then
      local mx,my=b,c
      if inRect(mx,my,backRect) then playSound("minecraft:block.note_block.pling",CONFIG.sfx*CONFIG.master); return end
      ...
      elseif inRect(mx,my,downBtn) then if offset+visibleRows < #names then offset=offset+1 end
        ...
        for i,bt in ipairs(buttons) do local x1b,y1b,x2b,y2b,lab=bt[1],bt[2],bt[3],bt[4],bt[5]
          if mx>=x1b and mx<=x2b and my>=y1b and my<=y2b then CONFIG.theme=lab; saveConfig(); playSound("minecraft:block.note_block.pling",CONFIG.sfx*CONFIG.master); break end
        end
      end
...
-- Allgemeiner Scroll-Text-Viewer für lange Listen (Journal/Inventar)
local function scrollTextPanel(title, lines)
  ...
  while true do
    ...
    if e=="monitor_touch" then
      local mx,my=b,c
      if inRect(mx,my,backRect) then return end
      -- scroll up/down areas (rechts; gleiche Koordinaten wie Buttons oben)
      ...
      elseif inRect(mx,my,dn) then if offset+maxRows < #lines then offset=offset+1 end
      end
    elseif e=="key" and a==keys.back then return end
  end
end
...
local function statsScreen()
  ...
  scrollTextPanel("Statistik (Lesen)", lines)
end
...
-------------------- DIALOG / STORY ENGINE -------------
local function drawLayoutScene(title,subtitle)
...
local function doChoice(prompt,options)
  ...
  local lines=wrap(prompt,width)
  -- Scroll falls Text länger ist:
  ...
  local function drawText()
    for row=0,tMaxRows-1 do
      ...
      if lines[idx] then term.setCursorPos(x1+2,textTop+row); term.write(lines[idx]) end
    end
    if #lines>tMaxRows then drawScrollButtons(th,x2,textTop,textBottom) end
  end
  drawText()
  ...
  while true do
    ...
    if e=="monitor_touch" then
      local mx,my=b,c
      -- Scroll-Klick im Text?
      if #lines>tMaxRows then
        local up={x2-4,textTop,x2-1,textTop+2}
        local dn={x2-4,textBottom-2,x2-1,textBottom}
        if inRect(mx,my,up) then if tOffset>0 then tOffset=tOffset-1; drawText() end
        elseif inRect(mx,my,dn) then if tOffset+tMaxRows<#lines then tOffset=tOffset+1; drawText() end
        end
      end
      -- Buttons?
      for i,bt in ipairs(buttons) do local x1b,y1b,x2b,y2b=bt[1],bt[2],bt[3],bt[4]
        if mx>=x1b and mx<=x2b and my>=y1b and my<=y2b then playSound("minecraft:block.note_block.pling",CONFIG.sfx*CONFIG.master); return i end
      end
    elseif e=="key" then
      if a>=keys.one and a<=keys.nine then return (a-keys.one)+1
      elseif a==keys.enter or a==keys.numPadEnter then return 1
      elseif a==keys.back then return 1
      end
    end
  end
end
...
local function puzzleLevers()
  drawLayoutScene("Puzzle: Hebel","Finde den richtigen")
...
local function puzzleRunes()
  drawLayoutScene("Puzzle: Runen","Ordne sie richtig")
...
local function scene_intro()
  drawLayoutScene("Die Flusternde Ruine","Willkommen, "..(state.playerName or ""))
  setMood("neutral")
...
local function scene_courtyard()
  drawLayoutScene("Hof der Schatten","Kuehler Wind")
...
local function scene_hall()
  drawLayoutScene("Halle der Echos","Das Summen wird lauter")
...
local function scene_riddle1()
  drawLayoutScene("Erste Frage","Pruefung des Verstandes")
...
local function scene_riddle2()
  drawLayoutScene("Zweite Frage","Wuerde")
...
local function scene_depth()
  drawLayoutScene("Die Tiefe","Nur schwaches Licht")
...
local function scene_secret()
  drawLayoutScene("Geheimes Kapitel","Der verborgene Hof")
...
local function scene_ending_secret()
  doEnding("good")
  drawLayoutScene("Geheimer Schwur","Ein letzter Schritt")
...
local function mainMenu()
  ...
  while true do
    ...
    makeButtons(x1,y1+2,x2,y2-4,opts); drawButtons(th)
    local _backRect=drawBackButton(th,x1,y2) -- no-op im Hauptmenue
