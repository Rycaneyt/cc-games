-- entertainment_story.lua  (v2.3 - ASCII safe)
-- CC:Tweaked / ComputerCraft – Multi-Monitor Text-Adventure
-- Features:
-- - Theme-Auswahl (persistiert)
-- - Auto-TextScale (oder manuell)
-- - Save/Load/Reset (Autosave nach jeder Entscheidung)
-- - Touch-Buttons + Tastatur (1..9/Enter)
-- - Animationen (Typewriter, Wipe, Confetti)
-- - Optionale Sounds (speaker)
--
...
if not mon then
  print("Kein Monitor gefunden! Bitte Advanced-Monitor anschliessen.")
  return
end
...
local CONFIG = { theme = "Ocean", textScale = 0 } -- textScale: 0 = auto

-- Save/Load fuer CONFIG
...
local function loadConfig()
  ...
  if ok and type(t)=="table" then for k,v in pairs(t) do CONFIG[k]=v end; return true end
  return false
end
loadConfig()
...
-- Auto-TextScale: sucht eine brauchbare Skalierung fuer grosse Monitore
...
local function writeCentered(y,text,fg,bg) setc(fg,bg) term.setCursorPos(cx(text),y) term.write(text or "") end
...
local function drawButtons(theme)
  for _, b in ipairs(buttons) do
    ...
    drawBox(x1, y1, x2, y2, theme.frame, theme.bg)
    local text = tostring(idx) .. ") " .. (label or "")
    local midY = math.floor((y1 + y2) / 2)
    setc(theme.btnNum, theme.bg)
    term.setCursorPos(cx(text), midY); term.write(tostring(idx))
    setc(theme.fg, theme.bg)
    term.setCursorPos(cx(text) + 2, midY); term.write(") " .. (label or ""))
  end
end
  ...
  while true do
    ...
    if e == "monitor_touch" then
      ...
      for _, bt in ipairs(buttons) do
        local x1, y1, x2, y2, _, idx = bt[1], bt[2], bt[3], bt[4], bt[5], bt[6]
        if mx >= x1 and mx <= x2 and my >= y1 and my <= y2 then return idx end
      end
    elseif e == "key" then
...
local function drawLayout(theme, title, left, right)
  ...
  writeCentered(1, title or "", theme.titleFg, theme.titleBg)
...
----------------------------------------------------------------
-- Story / Szenen (ASCII-sauber)
----------------------------------------------------------------
local function theme() return THEMES[CONFIG.theme] or THEMES.Ocean end

local scenes = {
  intro = {
    title="Die Flusternde Ruine",
    text ="Du stehst vor den Toren einer alten Ruine. Legenden berichten von einer Stimme, die nur denen antwortet, die aufrichtig fragen.",
    options={
      {label="Mutig eintreten",          effect=function() state.courage=state.courage+1 end, next="hall"},
      {label="Erst das Gelaende pruefen", effect=function() state.wisdom=state.wisdom+1 end, next="courtyard"},
    }
  },
  courtyard = {
    title="Hof der Schatten",
    text ="Im Hof findest du Inschriften, halb ueberwuchert. Eine Markierung zeigt nach Osten. Ein leises Summen steigt aus der Tiefe.",
    options={
      {label="Inschriften entschluesseln", effect=function() state.wisdom=state.wisdom+1 end, next="riddle1"},
      {label="Dem Summen folgen",           effect=function() state.courage=state.courage+1 end, next="hall"},
    }
  },
  hall = {
    title="Halle der Echos",
    text ="Deine Schritte hallen. In der Ferne glimmt blaues Licht. Eine steinerne Tuer fragt: \"Was suchst du?\"",
    options={
      {label="Macht",                 effect=function() end, next="riddle2"},
      {label="Wissen",                effect=function() state.wisdom=state.wisdom+1 end, next="riddle2"},
      {label="Heilung fuer andere",   effect=function() state.empathy=state.empathy+1 end, next="riddle2"},
    }
  },
  riddle1 = {
    title="Erste Frage",
    text ="\"Welche leise Kraft versetzt Mauern ohne Haende?\"",
    options={
  ...
  riddle2 = {
    title="Zweite Frage",
    text ="\"Was macht einen Boten wuerdig?\"",
    options={
      ...
      {label="Mitgefuehl",      effect=function() state.empathy=state.empathy+1 end, next="final"},
    }
  },
  final = {
    title="Entscheidung der Ruine",
    text = function() return ("Werte – Mut:%d  Verstand:%d  Mitgefuehl:%d"):format(state.courage,state.wisdom,state.empathy) end,
...
local function showScene(id)
  ...
  clear(th.bg)
  drawLayout(th, "ENTERTAINMENT STATION", ("Mut:%d  Verstand:%d  Mitgefuehl:%d"):format(state.courage,state.wisdom,state.empathy), "Touch: Auswahl | 1..9")
  ...
  if choice and choice.next then
    if choice.next == "ending" then
      ...
      if k=="good" then
        ...
        writeCentered(math.floor(H/2)-2, "Ein Tor aus Licht oeffnet sich.", th.good)
        writeCentered(math.floor(H/2),   "\"Wer fuer andere bittet, ist willkommen.\"", th.good)
        confetti(th, 2.2); jingle_success()
      elseif k=="neutral" then
        drawLayout(th, "ANTWORT DER RUINE", "Pfad des Verstehens", "")
        writeCentered(math.floor(H/2)-2, "Die Steine erzaehlen dir die Geschichte.", th.neutral)
        writeCentered(math.floor(H/2),   "Wissen wiegt schwer – trage es weise.", th.neutral)
        confetti(th, 1.2)
      else
        drawLayout(th, "ANTWORT DER RUINE", "Pfad des Mutes", "")
        writeCentered(math.floor(H/2)-2, "Die Tuer weicht, doch die Tiefe prueft dich.", th.bad)
        writeCentered(math.floor(H/2),   "Mut ohne Ziel ist nur Laerm im Dunkeln.", th.bad)
        confetti(th, 1.6)
      end
      writeCentered(H-3, "Tippen um erneut zu spielen", th.fg)
      saveGame()
...
local function themeMenu()
  ...
  while true do
    clear(th.bg)
    drawLayout(th, "THEME AUSWAeHLEN", "Touch/1..9 zum Wechseln", "Zurueck: [Q]")
    local x1,y1 = 3, FRAME.titleH+1
    local x2,y2 = W-2, H-FRAME.statusH-1
    local opt = {}
    for _,n in ipairs(names) do table.insert(opt, {label=n}) end
...
----------------------------------------------------------------
-- Einstellungen: Textgroesse (optional)
----------------------------------------------------------------
local function scaleMenu()
  ...
  while true do
    clear(th.bg)
    drawLayout(th, "TEXTGROeSSE", "Skalierung waehlen (0=Auto)", "Zurueck: [Q]")
    ...
    local c = waitChoice()
    if c==1 then
      CONFIG.textScale=0; saveConfig(); autoTextScale(); W,H=term.getSize(); return
    else
      local idx=c-1
      if scales[idx] then CONFIG.textScale=scales[idx]; saveConfig(); mon.setTextScale(scales[idx]); W,H=term.getSize(); return end
    end
  end
end
...
local function splash(th)
  ...
  writeCentered(math.floor(H/2)-1, "Die Flusternde Ruine", th.accent)
  writeCentered(math.floor(H/2)+1, "Tippen um zu beginnen", th.fg)
  while true do local e=os.pullEvent(); if e=="monitor_touch" or e=="key" then break end end
end
...
local function mainMenu()
  ...
  while true do
    th = theme()
    clear(th.bg)
    drawLayout(th, "HAUPTMENUE", "Waehle eine Option", "")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    ...
    table.insert(opts,{label="Theme auswaehlen"})
    table.insert(opts,{label="Textgroesse"})
    if hasSave then table.insert(opts,{label="Spielstand loeschen"}) end
    ...
    if hasSave and c==idx then
      jingle_click()
      clear(th.bg); drawLayout(th,"BESTAeTIGUNG","Spielstand loeschen?","1=Ja  2=Nein")
      makeButtons(3,FRAME.titleH+4,W-2,H-FRAME.statusH-1,{{label="Ja, loeschen"},{label="Nein, zurueck"}}); drawButtons(th)
      local cc=waitChoice()
      if cc==1 then deleteSave(); jingle_click(); writeCentered(math.floor(H/2), "Geloescht.", th.bad); sleep(0.6) end
