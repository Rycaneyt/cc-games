...
-- "Server Selbstzerstoerung" (Funmodus, blockiert am Ende), Spionage-Menue,
-- KILL (Simulation mit Doppelbestaetigung). ASCII-only, CC:Tweaked 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY   = "Koenig Fabio"
local VERSION            = "FabIO-AdminOS v3.1"
local INITIAL_SCALE      = 1.5 -- nur Monitor
...
-- Ehren-Titel (subtil)
local HONORIFY = {
  ["fabio"] = "Seine Exzellenz, Meister der Knoepfe",
  ["admin"] = "Hueter der Systeme",
...
if ok and found then mon = found; pcall(function() mon.setTextScale(INITIAL_SCALE) end) end
...
local function centerText(y, text) local tw=#text; local x=math.max(1, math.floor((w-tw)/2)+1); term.setCursorPos(x,y); term.write(text) end
...
local function applyScale() if mon then pcall(function() mon.setTextScale(currentScale) end) end; resizeCache(); os.queueEvent("term_resize") end
local function changeScale(delta) local ns=math.max(MIN_SCALE, math.min(MAX_SCALE, currentScale+delta)); if math.abs(ns-currentScale)>1e-6 then currentScale=ns; applyScale() end end
...
local function handleTouch(x,y) for _,a in ipairs(touchAreas) do if x>=a.x1 and x<=a.x2 and y>=a.y1 and y<=a.y2 then if a.handler then a.handler(a.id) end; return a.id end end; return nil end
...
-------------------- Logs --------------------
local logs = {}
local function addLog(line) local t=os.date("[%Y-%m-%d %H:%M:%S]"); table.insert(logs,1,t.." "..line); if #logs>300 then for i=301,#logs do logs[i]=nil end end end
...
local function aiSpeak(lines, delay) delay=delay or 0.7; for _,ln in ipairs(lines) do term.setTextColor(colors.yellow); print("> System: "..ln); term.setTextColor(colors.white); safePlayNote("C","harp",0.4); sleep(delay) end end
...
local function drawKeyboard(yStart)
  ...
  for r,row in ipairs(KB_ROWS) do
    term.setCursorPos(xStart,yStart+r-1); term.setTextColor(colors.lightGray)
    local x=xStart
    for i=1,#row do
...
local function keyboardHitToChar(x,y,yStart)
  ...
  for r,row in ipairs(KB_ROWS) do
    ...
    for i=1,#row do
      ...
      if y==yy and x>=x1 and x<=x2 then if ch=="<" then return "BKSP" elseif ch==">" then return "ENTER" elseif ch==" " then return "SPACE" else return ch end end
      xx=xx+#disp+1
    end
  end
  return nil
...
local function readLineTouch(promptX,promptY,masked)
  ...
  while true do
    term.setCursorPos(promptX,promptY); term.clearLine(); term.write(masked and string.rep("*",#buf) or buf)
    local ev,a,b = waitKeyOrTouch()
    if ev=="char" then if a=="+" then changeScale(SCALE_STEP) elseif a=="-" then changeScale(-SCALE_STEP) else buf=buf..a end
    elseif ev=="key" then
    ...
    elseif ev=="term_resize" then
      clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons(); term.setCursorPos(1,promptY-2); print(""); drawKeyboard(kbY); drawScaleLabelAndButtons()
    end
  end
end

-------------------- Mini-Utils --------------------
local function loadingDots(x,y,dur) dur=dur or 1.2; local s=os.clock(); local i=0; while os.clock()-s<dur do local dots=string.rep(".", (i%4)); term.setCursorPos(x,y); term.write(dots..string.rep(" ",4-#dots)); sleep(0.2); i=i+1 end end
...
local function pressToContinueRobust(y)
  clearTouchAreas() -- wichtig: alte Hitboxen loeschen, sonst "nichts anklickbar"
  local label="[ OK ]"
  centerText(y, label)
...
-------------------- Aktionen --------------------
local function serverSelfDestructMode()
  -- Doppel-Bestaetigung
  local ok = confirmDialog("Server Selbstzerstoerung", "Kritischen Vorgang vorbereiten?")
  if not ok then return end
  ok = confirmDialog("Bestaetigung erforderlich", "Bitte bestaetigen: Vorbereitungen fortsetzen.")
  if not ok then return end

  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4, "Server Selbstzerstoerung: Aktiv")
  ...
  pressToContinueRobust(h-2)  -- jetzt garantiert klickbar
end
...
local function hackMojangAnimation()
  ...
  for i=1,(w-10) do term.setCursorPos(5+i,y); term.setTextColor((i%2==0) and colors.green or colors.lightBlue); term.write("="); if i%4==0 then safePlayNote("C","harp",0.4) end; sleep(0.03) end
...
local function interaktiveTanzroutine()
  ...
  for r=1,6 do for i,fr in ipairs(frames) do term.setCursorPos(1,10); term.clearLine(); centerText(10,fr); safePlayNote("C","harp",0.4+0.04*i); sleep(0.25) end end
...
local function fileSystemScan()
  ...
  term.setCursorPos(3,12); if flagged>0 then print(string.format("Ergebnis: %d Hinweis(e) ohne Relevanz. Kein Eingriff erforderlich.", flagged)) else print("Ergebnis: Keine Auffaelligkeiten.") end
...
local function toolDancefloor()
  ...
  for row=8,12 do term.setCursorPos(3,row); for col=1,math.min(w-6,48) do term.write(((row+col)%2==0) and "#" or ".") end; sleep(0.05) end
...
local function utilitiesMenu()
  while true do
    ...
    for i,v in ipairs(items) do term.setCursorPos(4, baseY + i - 1); term.write(i..") "..v); addTouchArea(1, baseY + i - 1, w, baseY + i - 1, "util_"..i, function() end) end
    ...
    elseif ev=="monitor_touch" then local id=handleTouch(p2,p3); if id and id:match("^util_") then sel=tonumber(id:match("util_(%d+)")) end; if id=="font_minus" then changeScale(-SCALE_STEP) elseif id=="font_plus" then changeScale(SCALE_STEP) end
    elseif ev=="char" and (p1=="+" or p1=="-") then changeScale(p1=="+" and SCALE_STEP or -SCALE_STEP) end
...
-------------------- Spionage / Kill --------------------
local function spyLiveFeed(name)
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Spionage: Live-Feed ("..name..")")
  term.setCursorPos(3,6); print("- Verbinde Sensoren ..."); loadingDots(30,6,0.8)
  -- ASCII "Radar"
  for r=1,12 do
    term.setCursorPos(3,7); term.clearLine(); term.write("Radar: ")
    for i=1,math.min(w-12,50) do term.write(((i+r)%7==0) and "*" or ".") end
    sleep(0.06)
  end
  term.setCursorPos(3,9); print("- Position: "..name.." scheint sich unauffaellig zu bewegen.")
  term.setCursorPos(3,10); print("- Aktivitaet: Block betrachtet, nichts Verdächtiges.")
  addLog("[SPY] Live-Feed abgefragt fuer "..name..".")
  pressToContinueRobust(h-2)
end

local function spyVitals(name)
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Spionage: Vitaldaten ("..name..")")
  term.setCursorPos(3,6); print("- Synchronisiere Herzfrequenz ..."); loadingDots(32,6,0.9)
  -- Herzfrequenz "Graph"
  term.setCursorPos(3,8); print("HR: ")
  for i=1,math.min(w-8,48) do term.write((i%9==0) and "^" or "-"); sleep(0.03) end
  term.setCursorPos(3,10); print("Durchschnitt: "..math.random(58,92).." bpm")
  term.setCursorPos(3,11); print("Anmerkung: Kaffee-Level stabil.")
  addLog("[SPY] Vitaldaten ("..name..") abgerufen.")
  pressToContinueRobust(h-2)
end

local function spyInventory(name)
  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Spionage: Inventar-Snapshot ("..name..")")
  term.setCursorPos(3,6); print("- Analysiere Ausruestung ..."); loadingDots(30,6,0.8)
  local funItems = {"Schaufel Stufe Uebertreibung","Keks (krumelig)","Unbenannter Hebel","Mysteriöser Knopf","Brot (leicht angeknabbert)"}
  for i=1,#funItems do term.setCursorPos(3,7+i); print("- "..funItems[i]) end
  addLog("[SPY] Inventar-Snapshot fuer "..name.." erstellt.")
  pressToContinueRobust(h-2)
end

local function killPlayerSim(name)
  -- Doppel-Bestaetigung
  local ok = confirmDialog("KILL-Befehl", "Ziel: "..name.."\nFortfahren?")
  if not ok then return end
  ok = confirmDialog("Endgueltige Bestaetigung", "Sicher, dass "..name.." terminiert werden soll?")
  if not ok then return end

  clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
  centerText(4,"Ausfuehrung: KILL "..name)
  term.setCursorPos(3,6); print("- Sammle Kontext ..."); loadingDots(25,6,0.7)
  term.setCursorPos(3,7); print("- Pruefe Rechte ... OK")
  term.setCursorPos(3,8); print("- Sende Befehl an Endpunkt ..."); loadingDots(28,8,0.8)
  term.setTextColor(colors.magenta); centerText(10,"Richtlinie greift: Ausfuehrung blockiert."); term.setTextColor(colors.white)
  addLog("[SPY] Kill-Request fuer "..name.." blockiert (Richtlinie).")
  term.setCursorPos(3,12); print("Hinweis: "..name.." wurde stattdessen ueber Cookies informiert.")
  pressToContinueRobust(h-2)
end
...
local function killMenu()
  while true do
    clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
    centerText(4,"KILL (Simulation)")
    local baseY=6
    local items={"Ziel: Tainz","Ziel: Xisey","Zurueck"}
    for i,v in ipairs(items) do term.setCursorPos(4, baseY+i-1); term.write(i..") "..v); addTouchArea(1, baseY+i-1, w, baseY+i-1, "kill_"..i, function() end) end
    term.setCursorPos(4, baseY+#items+2); term.write("Zahl oder tippen. [+/-] Schriftgroesse")
    local ev,p1,p2,p3 = os.pullEvent()
    local sel=nil
    if ev=="char" and p1>='1' and p1<='3' then sel=tonumber(p1)
    elseif ev=="key" and p1==keys.enter then sel=3
    elseif ev=="monitor_touch" then local id=handleTouch(p2,p3); if id and id:match("^kill_") then sel=tonumber(id:match("kill_(%d+)")) end; if id=="font_minus" then changeScale(-SCALE_STEP) elseif id=="font_plus" then changeScale(SCALE_STEP) end
    elseif ev=="char" and (p1=="+" or p1=="-") then changeScale(p1=="+" and SCALE_STEP or -SCALE_STEP) end
    if sel==1 then killPlayerSim("Tainz")
    elseif sel==2 then killPlayerSim("Xisey")
    elseif sel==3 then return end
  end
end

local function spyMenuFor(name)
  while true do
    clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
    centerText(4,"Spionage: "..name)
    local baseY=6
    local items={"Live-Feed","Vitaldaten","Inventar-Snapshot","Zurueck"}
    for i,v in ipairs(items) do term.setCursorPos(4, baseY+i-1); term.write(i..") "..v); addTouchArea(1, baseY+i-1, w, baseY+i-1, "spy_"..i, function() end) end
    term.setCursorPos(4, baseY+#items+2); term.write("Zahl oder tippen. [+/-] Schriftgroesse")
    local ev,p1,p2,p3 = os.pullEvent()
    local sel=nil
    if ev=="char" and p1>='1' and p1<='4' then sel=tonumber(p1)
    elseif ev=="key" and p1==keys.enter then sel=4
    elseif ev=="monitor_touch" then local id=handleTouch(p2,p3); if id and id:match("^spy_") then sel=tonumber(id:match("spy_(%d+)")) end; if id=="font_minus" then changeScale(-SCALE_STEP) elseif id=="font_plus" then changeScale(SCALE_STEP) end
    elseif ev=="char" and (p1=="+" or p1=="-") then changeScale(p1=="+" and SCALE_STEP or -SCALE_STEP) end
    if sel==1 then spyLiveFeed(name)
    elseif sel==2 then spyVitals(name)
    elseif sel==3 then spyInventory(name)
    elseif sel==4 then return end
  end
end
...
local function spyMenu()
  while true do
    clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons()
    centerText(4,"Spionage")
    local baseY=6
    local items={"Tainz beobachten","Xisey beobachten","KILL ...","Zurueck"}
    for i,v in ipairs(items) do term.setCursorPos(4, baseY+i-1); term.write(i..") "..v); addTouchArea(1, baseY+i-1, w, baseY+i-1, "spym_"..i, function() end) end
    term.setCursorPos(4, baseY+#items+2); term.write("Zahl oder tippen. [+/-] Schriftgroesse")
    local ev,p1,p2,p3 = os.pullEvent()
    local sel=nil
    if ev=="char" and p1>='1' and p1<='4' then sel=tonumber(p1)
    elseif ev=="key" and p1==keys.enter then sel=4
    elseif ev=="monitor_touch" then local id=handleTouch(p2,p3); if id and id:match("^spym_") then sel=tonumber(id:match("spym_(%d+)")) end; if id=="font_minus" then changeScale(-SCALE_STEP) elseif id=="font_plus" then changeScale(SCALE_STEP) end
    elseif ev=="char" and (p1=="+" or p1=="-") then changeScale(p1=="+" and SCALE_STEP or -SCALE_STEP) end
    if sel==1 then spyMenuFor("Tainz")
    elseif sel==2 then spyMenuFor("Xisey")
    elseif sel==3 then killMenu()
    elseif sel==4 then return end
  end
end
...
local function showLogs()
  ...
  while true do
    ...
    for i=1,maxLines do
      ...
      if logs[idx] then local text=logs[idx]; if #text>w-4 then text=text:sub(1,w-7).."..." end; term.setTextColor(colors.white); term.write(text) end
    end
    term.setTextColor(colors.gray); term.setCursorPos(2, baseY + maxLines + 1); term.write("Pfeile scrollen, Enter/Zurueck-Button = zurueck, +/- Schriftgroesse")
    term.setTextColor(colors.white)
    ...
    elseif ev=="char" then if p1=="+" then changeScale(SCALE_STEP) elseif p1=="-" then changeScale(-SCALE_STEP) end
    elseif ev=="monitor_touch" then local id=handleTouch(p2,p3); if id=="logs_back" then return end end
  end
end
...
local function menuSelect(yFirst,count)
  while true do
    local ev,a,b=waitKeyOrTouch()
    if ev=="char" then if a>='0' and a<='9' then return a end; if a=="+" then changeScale(SCALE_STEP) elseif a=="-" then changeScale(-SCALE_STEP) end
    elseif ev=="key" then if a==keys.enter then return "\n" end; if a==keys.equals or a==keys.numPadAdd then changeScale(SCALE_STEP) elseif a==keys.minus or a==keys.numPadSubtract then changeScale(-SCALE_STEP) end
    elseif ev=="monitor_touch" then local id=handleTouch(a,b); if id~="font_minus" and id~="font_plus" then for i=1,count do if b==yFirst+(i-1) then return tostring(i) end end end
    end
  end
end
...
local function mainMenu()
  while true do
    ...
    local baseY=6
    local entries={
      "Server Selbstzerstoerung",      -- 1
      "Remote-Diagnose",               -- 2
      "Interaktives Modul (Test)",     -- 3
      "Logs anzeigen",                 -- 4
      "Dienstprogramme",               -- 5
      "Spionage",                      -- 6  <-- neu
      "Dateisystem-Scan",              -- 7
      "Geplanter Neustart",            -- 8
      "Beenden"                        -- 9 (bzw 0)
    }
    for i,v in ipairs(entries) do term.setCursorPos(4, baseY + i - 1); local num = (i==9) and "0" or tostring(i); term.write(num..") "..v); addTouchArea(1, baseY + i - 1, w, baseY + i - 1, "menu_"..i, function() end) end
    ...
    if     sel=="1" then serverSelfDestructMode()
    ...
    elseif sel=="9" or sel=="0" then
      addLog("[INFO] Sitzung beendet."); clearAndHeader(true); drawScaleLabelAndButtons()
...
local function loginIntro()
  ...
  while true do
    ...
    if ev=="monitor_touch" then local side,x,y = p1,p2,p3; if mon and side==peripheral.getName(mon) then local id=handleTouch(x,y); if id=="login_next" then break end end
    elseif ev=="timer" and p1==timerId then break
    elseif ev=="term_resize" then resizeCache(); clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons(); term.setTextColor(colors.lightBlue); centerText(3,"== AUTHENTICATION REQUIRED =="); term.setTextColor(colors.white); banner(4); drawLoginStatusPanel(8); btnX = math.floor((w - #label)/2)+1; btnY = h-2; term.setCursorPos(btnX,btnY); term.write(label); addTouchArea(btnX,btnY,btnX+#label-1,btnY,"login_next",function() end); centerText(h-1, hint)
    end
  end
end
...
local function loginForm()
  ...
  local uLower = tostring(user):lower(); local honor = HONORIFY[uLower]; if honor then addLog("[AUTH] Identitaetshinweis: "..honor) end
  ...
  print("Zugriff gewaehrt. "..greet); term.setTextColor(colors.white)
...
local function start()
  ...
  while true do
    ...
    if ev=="term_resize" then clearTouchAreas(); clearAndHeader(true); drawScaleLabelAndButtons(); centerText(3,"Systemkonsole"); centerText(5,"Weiter mit Enter oder durch Tippen") end
  end
  login()
  mainMenu()
end
