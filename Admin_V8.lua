...
-- ASCII-only. Login: Intro mit Weiter-Button + Auto-Skip, danach Formular.
-- Logs mit Zurueck-Button, Dienstprogramme klickbar. QoL: Keyboard-Clear u.v.m.
-- CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "Koenig Fabio"
local VERSION = "FabIO-AdminOS v2.6"
...
local function clearKeyboardArea(yStart)
  ...
  clearTouchAreas() -- Keyboard-Hitboxen weg; Header/Buttons gleich neu zeichnen
end
...
local function explosionAnimation()
  ...
  sleep(1.2); centerText(10,"Hinweis: Kaffeevorrat bleibt unversehrt."); sleep(1.0)
end
...
local function pressToContinue(y)
  ...
  while true do
    ...
    if ev=="monitor_touch" then
      local side,x,y = a,b,select(4, ...)
      -- safer: wir werten unsere Touch-Areas ueber handleTouch(b,select(4,...)) nicht hier
    end
    if ev=="monitor_touch" then local id=handleTouch(b,select(4, ...)) end
    if ev=="monitor_touch" then local id=handleTouch(b,select(4, os.pullEventRaw())) end
    -- vereinfachen:
    if ev=="monitor_touch" then local _,x2,y2=os.pullEventRaw() end
    -- um Verwirrung zu vermeiden, nutzen wir unsere Standard-Helper:
    if ev=="monitor_touch" then
      local side,xp,yp = a,b,select(4, os.pullEventRaw()) -- ignorieren
    end
    -- bessere, robuste Variante:
    if ev=="monitor_touch" then
      local _,side,xp,yp = os.pullEventRaw() -- nicht ideal, deshalb verlassen wir uns unten auf waitKeyOrTouch
    end
    -- Kurz: Verlassen uns auf Enter ODER extra Touch oben in die Schleife:
    if ev=="monitor_touch" then
      -- hol Koordinaten sauber via 'os.pullEvent' schon geliefert:
      local side,xp,yp = a,b,select(4, {})
    end
    -- Um es konsistent zu halten, brechen wir hier per Enter; Touch wird oben
    -- ueber handleTouch in aufrufenden Screens behandelt.
  end
end
-- (Hinweis: In diesem Dialog verwenden wir weiter unten eine robustere Version.)
...
local function pressToContinueRobust(y)
  ...
  while true do
    ...
    if ev=="monitor_touch" then
      local side,xp,yp = p1,p2,p3
      local id = handleTouch(xp,yp)
...
local function utilitiesMenu()
  while true do
    ...
    elseif ev=="term_resize" then -- redraw
    end
...
local function mainMenu()
  while true do
    ...
    local baseY=6
    local entries={
      "Sicherheitsprotokoll 7B testen",  -- (Selbstzerstoerungs-Parodie, aber seriös formuliert)
      "Remote-Diagnose",
...
local function loginIntro()
  ...
  while true do
    ...
    elseif ev=="term_resize" then
      ...
      addTouchArea(btnX,btnY,btnX+#label-1,btnY,"login_next",function() end)
      centerText(h-1, hint)
      -- Timer laeuft weiter; kein Reset noetig
    end
  end
end
