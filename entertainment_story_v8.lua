-- entertainment_v3.lua  (v3.2 - Audio Minus/Plus, Back-Fixes, More Themes, Sunset Orange)
...
local THEMES = {
  ...
  -- Sunset zurück auf orange (klassisch warm):
...
local function drawBackButton(th, x1, y2)
  local bx1,by1=x1,y2-2; local bx2=bx1+14; local by2=y2
  drawBox(bx1,by1,bx2,by2,th.frame,th.bg); term.setCursorPos(bx1+2,by1+1); setc(th.fg,th.bg); term.write("< Zurueck")
  return {bx1,by1,bx2,by2}
end
local function waitBackOrTouch(backRect)
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" then if b>=backRect[1] and b<=backRect[3] and c>=backRect[2] and c<=backRect[4] then return true end
    elseif e=="key" and (a==keys.back or a==keys.enter) then return true end
    end
  end
end
...
local function themeMenu()
  ...
  while true do
    clear(th.bg); drawLayout("Theme auswaehlen", "Tippe ein Theme oder < Zurueck", "")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
...
local function scaleMenu()
  ...
  while true do
    clear(th.bg); drawLayout("Textgroesse", "Tippe eine Option oder < Zurueck", "")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
...
-------------------- SETTINGS + AUDIO ------------------
-- Audio-Menü mit separaten Minus/Plus-Buttons und stabilem Zurück
...
local function audioMenu()
  local th=theme()
  while true do
    clear(th.bg); drawLayout("Audio","Tippe Minus/Plus, Mute, Zurueck","")
    local x1,y1=3,FRAME.titleH+1; local x2,y2=W-2,H-FRAME.statusH-1
    drawBox(x1,y1,x2,y2,th.frame,th.bg)
    local masterCtl = drawStepper(th, x1+2, y1+2, "Master", CONFIG.master)
    local musicCtl  = drawStepper(th, x1+2, y1+6, "Musik ", CONFIG.music)
    local sfxCtl    = drawStepper(th, x1+2, y1+10,"SFX   ", CONFIG.sfx  )
    ...
    while true do local e,a,b,c=os.pullEvent()
      if e=="monitor_touch" then local mx,my=b,c; playSound("minecraft:block.note_block.pling", CONFIG.sfx*CONFIG.master)
        -- Master
        if inRect(mx,my,masterCtl.minus) then CONFIG.master=math.max(0,CONFIG.master-0.1); saveConfig()
        elseif inRect(mx,my,masterCtl.plus) then CONFIG.master=math.min(1,CONFIG.master+0.1); saveConfig()
        -- Musik
        elseif inRect(mx,my,musicCtl.minus) then CONFIG.music=math.max(0,CONFIG.music-0.1); saveConfig()
        elseif inRect(mx,my,musicCtl.plus) then CONFIG.music=math.min(1,CONFIG.music+0.1); saveConfig()
        -- SFX
        elseif inRect(mx,my,sfxCtl.minus) then CONFIG.sfx=math.max(0,CONFIG.sfx-0.1); saveConfig()
        elseif inRect(mx,my,sfxCtl.plus) then CONFIG.sfx=math.min(1,CONFIG.sfx+0.1); saveConfig()
        -- Mute
...
local function statsScreen()
  ...
  local lines={
    ...
    "Endings: "..tostring((function() local c=0; for _ in pairs(state.endings) do c=c+1 end; return c end)()),
  ...
  local backRect = drawBackButton(th, 4, H-1)
  waitBackOrTouch(backRect)
end
  ...
  while true do
    ...
    while true do local e,a,b,c=os.pullEvent()
      if e=="monitor_touch" then
        ...
        elseif b>=backRect[1] and b<=backRect[3] and c>=backRect[2] and c<=backRect[4] then return
        end
      elseif e=="key" and (a==keys.enter or a==keys.numPadEnter) then if #state.playerName>0 then return end
      elseif e=="key" and a==keys.back then return
      end
    end
  end
end
  ...
  while true do local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" then local mx,my=b,c
      ...
      for idx,bt in ipairs(buttons) do local x1b,y1b,x2b,y2b=bt[1],bt[2],bt[3],bt[4]
        if mx>=x1b and mx<=x2b and my>=y1b and my<=y2b then
          playSound("minecraft:block.note_block.pling", CONFIG.sfx*CONFIG.master)
          if idx>=1 and idx<=3 then state.slot=idx; return
          elseif idx==4 then deleteSave(state.slot); return
          end
        end
      end
    elseif e=="key" and a==keys.back then return end
...
local function mainMenu()
  ...
  while true do
    ...
    makeButtons(x1,y1+2,x2,y2-4,opts); drawButtons(th)
    local backRect = drawBackButton(th, x1, y2) -- im Hauptmenue = "Zurueck" => kein Effekt (nur Konsistenz)
    showStatsSummary(6, H-5)
    local e,a,b,c=os.pullEvent()
    if e=="monitor_touch" then local mx,my=b,c
      -- Back im Hauptmenue ignorieren (bleibt im Menu)
      for idx,bt in ipairs(buttons) do local x1b,y1b,x2b,y2b=bt[1],bt[2],bt[3],bt[4]
        if mx>=x1b and mx<=x2b and my>=y1b and my<=y2b then
          playSound("minecraft:block.note_block.pling", CONFIG.sfx*CONFIG.master)
    ...
    elseif e=="key" then if a==keys.m then drawMap() end end
  end
end
