-- entertainment_v3.lua  (v3.4 – Back buttons everywhere, option scrolling, touch keyboard, playtime gating)
...
-- Themes (Sunset orange; Neon grell & sichtbar)
local THEMES={
...
local state={
  ...
  playTime=0,                        -- gesammelte Story-Spielzeit (Sekunden)
  _playTimerOn=false, _playTimerT0=0, -- interner Laufzeitmesser
  mapLoc="Gate",
...
-------------------- BUTTONS / TOUCH -------------------
-- Allgemeine Touch-Buttons
local buttons={}
local function _truncateToWidth(s, w)
  if #s<=w then return s end
  if w<=1 then return string.sub(s,1,1) end
  return string.sub(s,1,math.max(0,w-1)).."…"
end
...
local function drawButtons(th)
  for _,b in ipairs(buttons) do
    ...
    local maxLabel = math.max(1, (x2 - x1) - 6) -- Platz für "n) "
...
local function save(slot)
  -- Playtime nur Inside-Story sammeln (Timer evtl. läuft)
  playTimerStop()  -- bevor geschrieben wird, konsolidieren
...
local function load(slot)
  ...
  if ok and type(t)=="table" and t.state then
    ...
    state.slot=slot
    state._playTimerOn=false; state._playTimerT0=0
    return true
  end
  return false
end
...
local function drawMap()
  ...
  local backRect=drawBackButton(theme(),x1,y2); waitBackOrTouch(backRect)
end
...
local function themeMenu()
  ...
  local offset=0
  local visibleRows=math.max(1, math.floor((listBottom-listTop+1)/3))
  while true do
    ...
    if e=="monitor_touch" then
      ...
      elseif inRect(mx,my,downBtn) then if offset+visibleRows<#names then offset=offset+1 end
        ...
        for _,bt in ipairs(buttons) do if mx>=bt[1] and mx<=bt[3] and my>=bt[2] and my<=bt[4] then CONFIG.theme=bt[5]; saveConfig(); break end end
      end
...
local function statsScreen()
  ...
  local lines={
    ...
    "Spielzeit (Story): "..formatTime(played),
  }
  scrollTextPanel("Statistik", lines)
end
...
local function touchKeyboard(prompt, initial)
  ...
  while true do
    clear(th.bg); drawLayout(prompt or "Eingabe","Tippe Tasten | OK","")
    local x1,y1=8,FRAME.titleH+2; local x2=W-8; local y2=H-FRAME.statusH-1
    drawBox(x1,y1,x2,y1+2,th.frame,th.bg); term.setCursorPos(x1+2,y1+1); setc(th.fg,th.bg); term.write(_truncateToWidth(text, x2-x1-4))
    local keyW=8; local keyH=3; local gx=x1; local gy=y1+5
    local keys={}
    for row=1,#KB_ROWS do
      gx=x1
      for col=1,#KB_ROWS[row] do
        local label=KB_ROWS[row][col]
        local kx1,ky1,kx2,ky2=gx,gy,gx+keyW-1,gy+keyH-1
        drawBox(kx1,ky1,kx2,ky2,th.frame,th.bg)
        term.setCursorPos(kx1+2,ky1+1); term.write(_truncateToWidth(label, keyW-3))
...
local function askName()
  ...
  if input and #input>0 then state.playerName=input end
end
...
-- Optionen-Seiten mit Scroll-Pfeilen
local function optionMenuPaged(x1,y1,x2,y2,options)
  ...
  local function render()
    ...
    for i=1,rows do
      local idx=offset+i
      if options[idx] then table.insert(view, options[idx]) end
    end
    makeButtons(x1,y1,x2-6,y2,view); drawButtons(th)
    drawScrollButtons(th,x2,y1,y2)
  end
  while true do
    render()
    local e,a,b,c=os.pullEvent()
...
local function mainMenu()
  local th=theme()
  playTimerStop() -- sicherstellen, dass im Hauptmenue nicht gezählt wird
  ...
  while true do
    ...
    makeButtons(x1,y1+2,x2,y2-4,opts); drawButtons(th)
    local _backRect=drawBackButton(th,x1,y2)
...
local function game()
  ...
  introCutscene()
  playTimerStart()  -- ab hier zählt die Story-Zeit
  while true do
    ...
    while true do
      ...
      elseif e=="key" and (a==keys.p) then playTimerStop(); local r=pauseMenu(); if r=="quit" then return end; playTimerStart(); break
...
while true do
  local act=mainMenu()
  if act=="new" then
    state={slot=state.slot,playerName="",scene="intro",courage=0,wisdom=0,empathy=0,level=1,xp=0,flags={},decisions=0,endings={},playTime=0,_playTimerOn=false,_playTimerT0=0,mapLoc="Gate",journal={summary={},quests={},loreUnlocked={},notes={}},inventory={},achievements={},autosave=true,lastRecap="",portraitMood="neutral",portraitTick=0}
    save(state.slot)
