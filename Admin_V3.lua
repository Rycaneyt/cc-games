...
-- ASCII-only (keine Umlaute / Sonderzeichen), "Hacker"-Login mit Status-Panel.
-- CC:Tweaked / Minecraft 1.20.x

-------------------- Konfiguration --------------------
local USERNAME_DISPLAY = "Koenig Fabio"
local VERSION = "FabIO-AdminOS v2.1"
local INITIAL_SCALE = 2.0     -- Start-Schriftgroesse NUR fuer Monitor (Terminal bleibt nativ)
...
local function registerFontButtons()
  local label = "[ - ] [ + ]"
  local x = w - #label
  addTouchArea(x+2, 1, x+4, 1, "font_minus", function() changeScale(-SCALE_STEP) end)
  addTouchArea(x+8, 1, x+10, 1, "font_plus",  function() changeScale(SCALE_STEP) end)
end
...
addLog("[LOG] Fabio lachte 3x laut waehrend einer Systemwartung.", true)
...
addLog("[INFO] Netzwerkstatus: Verbunden (lokal).", false)
...
-------------------- "Hacker"-Status Panel fuer Login --------------------
...
local function explosionAnimation()
  ...
  addLog("[ACTION] Versuch Selbstzerstoerung initiiert (Simuliert).", true)
end
...
local function hackMojangAnimation()
  ...
  addLog("[SEC] Mojang breach: nur Katzenvideos gefunden.", true)
  sleep(1)
end
...
local function sillyOptions()
  ...
  while true do
    ...
    if ev == "char" and (a == '+' or a == '-') then changeScale(a=='+' and SCALE_STEP or -SCALE_STEP); clearTouchAreas(); sillyOptions(); return end
  end
end
...
local function fakeRestart()
  ...
  addLog("[INFO] System Neustart ausgefuehrt (fake).", false); sleep(1)
end
...
local function corruptFiles()
  ...
  addLog("[WARN] Scan peinliche Dateien: 0/0 (fehlerhaft).", true); sleep(1.2)
end
...
-------------------- Login (mit Status-Panel + On-Screen-Keyboard) --------------------
