-- entertainment_story.lua  (v2.2 - Umlaut-Fix + Fixed Buttons)
...
-- - Umlaut-Autokorrektur (äöüÄÖÜß + Mojibake "Ã¤" etc.)
--
...
----------------------------------------------------------------
-- Umlaut-Fix (UTF-8 & Mojibake -> CP437)
----------------------------------------------------------------
local function U(s)
  if not s or s=="" then return s or "" end
  -- Erst Mojibake-Sequenzen (Clipboard UTF-8 als Latin-1 gelesen):
  s = s
    :gsub("Ã¤", string.char(132))  -- ä
    :gsub("Ã¶", string.char(148))  -- ö
    :gsub("Ã¼", string.char(129))  -- ü
    :gsub("Ã„", string.char(142))  -- Ä
    :gsub("Ã–", string.char(153))  -- Ö
    :gsub("Ãœ", string.char(154))  -- Ü
    :gsub("ÃŸ", string.char(225))  -- ß
  -- Dann echte UTF-8-Umlaute (falls der Editor sie korrekt einfügt):
  s = s
    :gsub("ä", string.char(132))
    :gsub("ö", string.char(148))
    :gsub("ü", string.char(129))
    :gsub("Ä", string.char(142))
    :gsub("Ö", string.char(153))
    :gsub("Ü", string.char(154))
    :gsub("ß", string.char(225))
  return s
end
...
-- Auto-TextScale: sucht eine brauchbare Skalierung für große Monitore
...
local function writeCentered(y,text,fg,bg)
  text = U(text or "")
  setc(fg,bg); term.setCursorPos(cx(text),y); term.write(text)
end
local function wrapText(s,width)
  ...
  -- Umlaut-Fix pro Zeile
  for i=1,#lines do lines[i]=U(lines[i]) end
  return lines
end
...
----------------------------------------------------------------
-- Buttons / Eingabe  (KORRIGIERT + Umlaut-Fix)
----------------------------------------------------------------
local buttons = {} -- jedes Element: {x1, y1, x2, y2, label, idx, hint}
...
local function makeButtons(x1, y1, x2, y2, options)
  ...
  for i, opt in ipairs(options) do
    ...
    local label = U(opt.label or "")
    table.insert(buttons, {x1, y, x2, bY2, label, i, opt.hint})
    y = bY2 + 1
  end
end
...
local function waitChoice()
  while true do
    ...
    elseif e == "key" then
      if a >= keys.one and a <= keys.nine then
        return (a - keys.one) + 1
...
local function drawLayout(theme, title, left, right)
  ...
  setc(theme.statusFg, theme.statusBg); term.setCursorPos(2,H-1); term.write(left)
  if #right>0 then term.setCursorPos(W-#right-1, H-1); term.write(right) end
end
